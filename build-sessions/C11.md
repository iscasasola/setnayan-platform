...
and half the saving is a whole peso in every tier — so the midpoint between retail and sign-up is
already an exact charm price and there is nothing to round:
...
⚠ WHY NOT A PERCENTAGE: the prices carry charm endings, so the implied discounts are **40.02 · 40.03
· 40.04 · 40.20**, not a clean 40. Half of those is 20.01–20.10, which means a hard-coded `20` is
subtly wrong per tier TODAY, before any reprice. Computing in pesos avoids the question entirely —
there is no rounding step to get wrong and no display-vs-money-path divergence to police. If a
screen wants to SAY "20% off", that is a display-layer rounding of a number the money path never
uses.
