...
| SUP-52 (ANS-03) | **DONE** | #5601 shipped the guest-facing entry point: `app/[slug]/_components/song-request-card.tsx` + `app/api/song-requests/route.ts`, gated on the band's own `song_desk` read access. `lib/answers-desk.ts`'s `ANSWERS_THAT_DO_NOT_JOIN` no longer carries `song_request` (S25). | — |
