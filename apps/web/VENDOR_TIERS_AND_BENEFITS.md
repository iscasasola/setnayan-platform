...
- **Marketing/strategy session** — owns `app/vendors/*`, pricing copy, the benefit catalog + tier allocation below.
...
### ✦ CUSTOM — 🔴 RETIRED 2026-08-27 (owner) · was "Talk to us" (negotiated · from ₱8,999/28d)

> 🔴 **RETIRED BY OWNER RULING, 2026-08-27. ENTERPRISE IS NOW THE TOP PURCHASABLE
> TIER.** All six Custom catalogue rows (`vendor_custom_base` ₱8,999 ·
> `vendor_custom_reach_nationwide` ₱2,499 · `vendor_custom_domain` ₱499 ·
> `vendor_custom_event_slot` ₱499 · `vendor_custom_reach_step` ₱499 ·
> `vendor_custom_photo_pack` ₱99) are `is_active = false` as of migration
> `20271171000513`. Anyone needing more than Enterprise's caps is handled **by
> hand, off-platform** — there is no priced ladder above Enterprise any more.
>
> 🔒 **The tier CONCEPT is not deleted.** `vendor_tier_rank()` still ranks
> `custom` at 6 and the `vendor_tier_state` enum still carries the value —
> retiring what can be BOUGHT was the ruling; removing the tier would put the
> no-silent-downgrade guard at risk for nothing. Production held **zero** vendors
> on custom when this was applied (two profiles, both `solo`).
>
> 🚨 **AND THE FLAG ALONE DOES NOT CLOSE THE DOOR — this is unfinished, not done.**
> `lib/vendor-custom-catalog.ts` substitutes a hardcoded literal for any of these
> rows that goes missing, and its own docblock says deactivating a row *"is not a
> retirement"*. The vendor-side configurator is still linked from
> `/vendor-dashboard/subscription` and still quotes the same numbers. Closing it
> for real means deleting the axes from `CUSTOM_SKU_CODES` and
> `CUSTOM_UNIT_PRICE_FALLBACK`. **Everything below this banner is HISTORY — the
> record of how Custom was designed and priced. Do not read it as live.**
...
## 3 · Copy corrections already applied to `/vendors`
...
1. **Solo linchpin — RESOLVED by §6 audit: FALSE in code.** `solo.inAppGated=true` → Solo burns tokens like Pro/Ent. Owner decides: (a) re-pitch Solo as "unlimited volume + 3 services/category + expanded reach & analytics" (recommended — matches shipped code), or (b) set `solo.inAppGated=false` to make it token-free. Until decided, do **not** ship "Solo = no tokens" copy. **⚠ "real name day-1" is no longer a Solo differentiator** — the "open it up" lock retired the name paywall, so Verified shows its real name day-1 too.
...
3. **Solo has zero uplift over Verified** on portfolio (50=50), slots (1=1), radius (20=20) — monotonic but flat. Owner: bump one axis for Solo or lean the pitch on volume + servicesPerLeaf 3. (Name-reveal can no longer carry this — it's ungated on every tier now.)
4. **Verification pass — DONE** (§6). Remaining roadmap items must not be published as live: see §6 "ROADMAP".
5. Rebuild `/vendors` to the **benefits-forward, Free-in-the-spotlight** structure using §6 as the tier truth (still pending).
...
- **2026-07-22 · "OPEN IT UP" — name paywall RETIRED (build PR `claude/council-fix-names`).** Owner lock (Vendor_Subscription_Ladder_2026-07-22 §3): *a vendor's NAME is never gated.* Shipped: (1) `TIER_CAPS.verified.nameMode 'screen'→'true'` (removes the anonymized-until-reply/upgrade name paywall on the legacy free-verified tier); (2) the real reveal is now **verification-driven** — `resolveVendorDisplayName`/`isVendorNameRevealed` take a new `is_verified` input and reveal on `verification_state='verified'` on ANY tier (a verified vendor sits at `tier_state='free'`, so tier alone couldn't reveal them); threaded through all 17 name-consumer surfaces (explore card + folder strip, `/v/[slug]` + booth, couple category-search + bench-search, saved/library + shortlist picks, messages, showcase/creator/trusted-by). (3) Fixed the `vendor-cards.ts`/`saved-vendors.ts` `PAID_TIERS={pro,enterprise,custom}` hardcode → derive from `isTrueNameTier` (also un-anonymizes Solo, §6 miss). **Conservative edge:** `free.nameMode` stays `'hidden'` + `free.marketplaceSearchable` stays `false` (unchanged) — an UNVERIFIED vendor never reveals (is_verified false), which is the safety property that let the de-gate ship even on the surfaces that don't verification-gate their query. **Owner notes flagged:** (a) real verified vendors currently carry `tier_state='free'`; the reveal works because it keys on verification, not tier — but if a "verified tier_state" convention is ever wanted, that's a separate data move; (b) `bench-marketplace-search` is NOT verification-gated at the query layer (it shows unverified vendors as *placeholders*) — pre-existing PR-B gap, name-safe now but the owner may want to add the `verification_state='verified'` filter; (c) `VENDOR_TIER_SEARCH_GATE` (tier-based existence exclusion) contradicts the lock and should be retired, not flipped on. Copy corrected: §1/§4/§6 "real-name-day-1 as Solo's edge" claims + `public/llms.txt` vendor-tier lines + the "why is a name hidden" FAQ.

- **2026-07-01 · marketing session** — created this doc; captured the full tier allocation + 88-benefit catalog + naming decisions. Applied the `/vendors` copy corrections in §3. **For the dashboard session:** the tier→feature gates in §2 are the intended entitlement map — when you wire `vendor-tier-caps`/entitlement checks, mirror the "starts at" tiers here, and flag back any surface whose real gate differs so we reconcile. The Solo "unlimited answering" linchpin (§4.1) is still owner-unconfirmed — don't hard-code it yet.
...
- **2026-08-27 · price-sheet session — CUSTOM TIER RETIRED; ENTERPRISE IS THE TOP PURCHASABLE TIER (owner).** All six `vendor_custom_*` catalogue rows set `is_active = false` (migration `20271171000513`). Anyone above Enterprise's caps is handled by hand, off-platform. `vendor_tier_rank()` and the `vendor_tier_state` enum are deliberately **untouched** — retiring what can be bought is the ruling; deleting the tier is not. Verified against production first: **zero** vendors on custom (two profiles, both `solo`), so no rank can move. Same session repriced Enterprise **₱8,000 → ₱10,000 / 28d** and **₱80,000 → ₱104,000 / yr**, Solo annual **₱10,000 → ₱10,400**, Pro annual **₱25,000 → ₱26,000**, additional branch **₱999 → ₱1,000**, 3D Booth **₱1,500 → ₱2,500**. Every annual figure is exactly `28-day × 10.4` (thirteen periods, 20% off) — recorded as an observation, never encoded. **⚠ STILL OPEN, reported not built:** the `is_active` flag does NOT close the Custom door — `lib/vendor-custom-catalog.ts` falls back to hardcoded literals for a missing row and the configurator is still linked from the subscription page; closing it means deleting the axes from `CUSTOM_SKU_CODES` + `CUSTOM_UNIT_PRICE_FALLBACK`. **⚠ ALSO OPEN:** the four annual ADD-ON prices the owner's sheet gives (branch ₱10,400 · seat ₱2,600 · Vendor AI ₱15,600 · 3D Booth ₱26,000) were **not created** — every add-on term is a hardcoded 28 days and `create_vendor_subscription` raises `UNMAPPED_SKU_TIER` for any non-tier SKU, so a priced annual add-on row could take money and grant a 28-day window.
...
² **Search-existence gate contradicts the lock (owner note):** `VENDOR_TIER_SEARCH_GATE` would exclude `tier_state='free'` from marketplace search. It is env-OFF today, but the lock says paid tiers buy *prominence/reach, never existence* — so this tier-based existence gate should be **retired**, not flipped on. Left dormant + flagged here; not changed by the open-it-up PR.
...
- ~~**`vendor-cards.ts` PAID_TIERS={pro,enterprise} excludes Solo** from day-1 name reveal, contradicting `solo.nameMode='true'`.~~ **RESOLVED (open-it-up PR):** `vendor-cards.ts` + `saved-vendors.ts` now derive `isPaidTier` from `isTrueNameTier(tier)` (`tierCaps(tier).nameMode==='true'`), so Solo (and de-gated Verified) reveal day-1.
...
3. **Solo flat vs Verified** on portfolio/slots/radius — bump one axis or lean the pitch on volume + servicesPerLeaf 3 + expanded reach/analytics. (Name-reveal is off the ladder — ungated on every tier per the "open it up" lock.)
...
## 11 · Custom tier rate card — 🔴 RETIRED 2026-08-27 (history only)

> 🔴 **THE WHOLE OF §11 IS HISTORY.** The Custom tier was retired by owner ruling
> on 2026-08-27; Enterprise is now the top purchasable tier. The rate card below
> is kept because it records how the numbers were reasoned and signed, not
> because any of it is on sale. See the banner on the §2 Custom card for what was
> actually changed, and for the one thing that is still open.

_Original heading: Custom tier rate card (PROPOSED 2026-07-03 · owner sign-off pending)_
...
- **Feature entitlements (owner 2026-07-04): Custom runs as Enterprise AUTOMATICALLY.** One rule, not a parallel feature list: for every boolean/feature gate (name reveal · review comments · editorial tagging · custom slug · multi-admin governance · Flagship template · films rack · Demand Radar/Theft Watch/etc.), `custom` resolves to Enterprise-level entitlement; ONLY the numeric caps are overridden by the purchased composition. Implementation shape: `tierCaps('custom')` = Enterprise caps cloned with the composition's numbers (or an `effectiveTier()` that maps custom→enterprise for feature checks). **⚠ Build-audit item: sweep for hard `tier === 'enterprise'` equality checks** (PAID_TIERS-style sets, `buyExtraSeat` Enterprise-only, Flagship template gate, films-rack cap) and convert to tier-rank/caps-derived checks so Custom inherits without per-surface patches — the same class of bug as the `vendor-cards.ts` PAID_TIERS/Solo miss in §6.
...
1. Merge #2623 (armed). 2. `TIER_CAPS` enterprise `serviceRadiusKm` ∞ → **100** (and sweep marketing copy for Enterprise "nationwide" claims — `/vendors`, pricing matrix, overlay). 3. Stage-2 configurator build (both surfaces per the Stage-2 spec above) — Opus brief goes out once #2623 + this PR land.
