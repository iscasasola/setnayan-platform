# BA3 — the plan and the ledger finally meet

**Model:** Opus 5 · **Effort:** high · **Wave:** after BA2 · **Blocks BA4 and BA6**
...
The centre of the whole rebuild. No migration.

## What already exists — find it before you build it

- `EventMoney.byBucket` (`lib/budget-truth.ts`) already computes, PER PLAN GROUP:
  `committedPhp · paidPhp · stillOwedPhp · overpaidPhp · estimatedPhp · hasBenchmark · benchmarkPhp`.
  Grep for `byBucket` across `apps/web` — outside tests and `scripts/budget-parity.ts` it has ZERO
  readers. It is computed on every budget page load and thrown away.
- `computeBudgetOverspend` (`lib/budget-overspend.ts`) is caller-agnostic by design: it takes
  `{ benchmarkPhp, actualPhp }` per category and returns the overspend plus a greedy absorption
  plan naming which under-budget categories can cover it. Its ONLY caller today passes the couple's
  SLIDER as `actualPhp`, never what they actually agreed.
- The keys already match. `resolveAllocationInputs` sets `canonicalService: b.plan_group_id`, and
  `MoneyBucket.bucketId` IS the plan-group id. One namespace, never joined.

## Build

- One row per plan group: **Planned · Agreed · Paid · Owed**, with a bar showing paid / owed and
  the overage past the plan mark.
- Feed `byBucket` into `computeBudgetOverspend` as `actualPhp`, replacing the slider.
- Absorption plan behind a tap, not on the row.
- §18.5 rule 5: a leaf with no seeded benchmark must print "no typical price yet", NEVER ₱0.
  Cake has no benchmark in production and money can still land there — render Planned as "—" and
  mark the row unplanned rather than inventing a zero.

## Guards to add

- Fails if a bucket carrying money renders no row.
- Fails if a category with `hasBenchmark === false` prints a ₱0 planned figure.
- Fails if the four column labels are abbreviated or reordered.

## Done when

A couple who planned ₱450,000 for catering and signed a ₱480,000 caterer sees that said on the row,
and the absorption plan names the categories with the headroom to cover it.
