# X0 — calendar-first — nothing here waits for anything else

**Model:** Sonnet 5 · **Effort:** medium · **Wave:** day 1 · owner + ½ day eng · runs alongside S0

Measured against `origin/main` @ `bd1c1a39f` on 2026-09-05. Re-fetch before you act.

Part of the S-series encoder plan — `build-sessions/encoder/README.md`. Supersedes E7's calendar half.
...
TASK — X0: start every lead-time item today, so no later session is blocked by paperwork.

OWNER ITEMS (the session cannot do these; it opens the tracking issue and records the state):
  1. Apple: accept the updated Program License Agreement at developer.apple.com → Agreements.
     Membership is PAID (owner, 2026-09-05). Both `build-desktop` dispatches on 2026-09-05
     (02:03Z, 02:52Z) failed ONLY at notarization: HTTP 403 "A required agreement is missing or
     has expired". Windows leg passed both times. After accepting, re-dispatch and confirm the
     macOS leg notarizes — that closes S0 question 2 with no engineering.
  2. Apple identity: is the Developer ID under the PH organisation (needs D-U-N-S, ~15 business
     days) or the individual "Indalecio Casasola" the CI log shows? Record which. The answer
     decides whose name is on every couple's Gatekeeper dialog.
  3. Windows: order an OV code-signing certificate WITH cloud signing from a CA that validates PH
     companies — SSL.com OV (~$129/yr + eSigner metering) or Certum. Azure Trusted/Artifact Signing
     EXCLUDES Philippine organisations (learn.microsoft.com, 2026-08-11). Keys must be in hardware
     or cloud since 2023-06-01, so a file-based cert cannot be used from GitHub Actions.
     Charged at order; validation 3–4 weeks. Owner ruling: no waiting — order now.
  4. Vercel → Environment Variables: record the production values of
     NEXT_PUBLIC_LIVE_STUDIO_ROAM_ENABLED · NEXT_PUBLIC_PANOOD_STREAMING_ENABLED ·
     NEXT_PUBLIC_LIVE_STUDIO_POOL_ONLY. They are in neither the repo nor `get_project`. Without
     the first two the controller returns notFound() and no camera feed opens.

ENGINEERING (½ day):
  · Open ONE GitHub issue "Encoder program — lead-time tracker" listing the four items above with
    owner, date started, date needed (Windows cert in hand before S11), and status. Link it from
    `build-sessions/encoder/README.md`.
  · Verify what the Windows leg actually produced on the 02:52Z run: `gh run view 33940303449`
    → download the `.msi` artifact, record its size and whether it is signed
    (`signtool` not needed — `osslsigncode verify` or just note "unsigned" from the workflow).
  · Confirm the macOS notarization failure text verbatim from the log and paste it into the issue.

HAND BACK (format in rule 16): the issue number, the four owner items with state, and whether the
re-dispatch after the agreement was accepted notarized. PR only if README needed the link.
