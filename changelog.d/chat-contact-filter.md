...
**Detector — `apps/web/lib/chat-contact-filter.ts` (pure, 19 unit tests).** `evaluateMessage(body)` → `{ blocked, categories[], matched[] }`. Evasion-resistant PHONE detection: normalize spelled-out digits → numerals, then collapse short filler (spaces/punctuation/brief words, ≤20 chars) BETWEEN digits, then match PH mobile / `+63` / `09XX` / no-leading-0 shapes — so `0917 880 7163`, `0 9 1 7 8 8 0 7 1 6 3`, `0917 my number is 8807163`, `zero nine one seven…`, and `+63…` all resolve to the same number and block, while two numbers separated by a whole clause ("150 guests … 80000 budget") stay separate and pass. Plus email (+`(at)/(dot)`), social/messaging URLs, `@handle`, and an **editable `BLOCKLIST` array** = the "list of text that isn't allowed" (app names incl. word-boundaried `ig`/`fb`, colour euphemisms `blue/purple/green/pink app`, solicitations). One place to tune what the chatroom blocks.
