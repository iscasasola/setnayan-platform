# L5 — the consent screen, the policy and the code agree

**Model:** Sonnet 5 · **Effort:** medium · **Wave:** now · **Independent of L1**

Measured against `origin/main` @ `7d6206679` and the live site on 2026-09-01. Re-fetch before you act.
...
TASK — L5: make Google verification a formality, and pin the claims that could rot.

WHAT IS ALREADY TRUE — verified against the live site on 2026-09-01, do not rebuild:
  · /privacy returns 200 and carries a dedicated 'Google / YouTube data (Live Studio)' section
  · The Limited Use commitment is stated explicitly
  · /terms returns 200
  · Search Console domain ownership is live in DNS: a google-site-verification TXT record on
    setnayan.com
  · The project is External + In production, unverified, 3 of 100 lifetime cap used — all three
    consents are the owner's own

WHAT IS NOT VERIFIED and needs a human in the console (report, do not guess): consent-screen
branding, authorized domains, and whether the scope list there is exactly `auth/youtube`.

SCOPE — three guards, because each of these claims is one edit away from becoming false:

  1. PIN THE SCOPE LIST. The privacy policy's scope list must byte-match YOUTUBE_OAUTH_SCOPES.
     lib/panood-youtube.ts already carries the instruction; nothing enforces it. A policy that
     discloses a scope we do not request, or omits one we do, is a verification rejection and a
     compliance defect. Write the test that fails when they diverge.

  2. PIN THE FILM AGAINST ENTITLEMENT EXPIRY. watchFilmEmbedUrl is documented as 'fail-closed on all
     three' — a valid panood_watch_url AND an active Live Studio SKU. Confirm that a one-time PAID
     LIVE_STUDIO order keeps that section alive INDEFINITELY, and pin it. If entitlement can ever
     lapse, a couple's wedding film silently disappears from their own website years later and
     nobody would notice until they complained.

  3. RECORD WHY VERIFICATION MAY BE UNNECESSARY. Papic's Drive integration requests `drive.file`
     ONLY, which Google classifies as NON-SENSITIVE — so Drive sync is uncapped and needs no
     verification, and Google sign-in is uncapped too. The 100-user cap binds ONLY `auth/youtube`,
     i.e. only couples connecting their own channel. Put that in the corpus so no future session
     re-derives it or panics about the cap.

⚠ ALL THREE OAUTH CLIENTS SHARE ONE PROJECT — client IDs all begin `330619827695-` (Live Studio,
Drive, and the web sign-in client). The cap is per project, not per client. Internal/Workspace is
therefore OFF THE TABLE: switching the project to Internal would refuse every external user on the
SIGN-IN client, breaking 'Continue with Google' platform-wide.

⛔ DO NOT submit anything to Google. The demo video does not exist yet and cannot until L1 lands.
This session prepares; the owner submits.
