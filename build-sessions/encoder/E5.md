# E5 — RTMP and FLV, in Rust

**Model:** Opus 5 · **Effort:** high · **Wave:** parallel with E1–E4 (needs only the IPC contract)
...
TASK — E5: the one thing a browser cannot do — speak RTMP.

This is a BUILD-OR-BUY decision as much as systems code. Make it explicitly, in writing, before
writing the implementation.

  · **Native Rust RTMP client + FLV mux** — ~4–6 days. No bundle cost, no licence question,
    and you own the failure modes.
  · **ffmpeg sidecar** — ~2–3 days, but **+~80 MB of bundle** and an **LGPL/GPL call you are
    making for the whole product**, on both platforms, forever. Cheaper this week, and a
    licence position you cannot casually reverse.

⚠ CHECK THE LICENCE QUESTION WITH THE OWNER RATHER THAN ASSUMING. It is a product decision
wearing an engineering hat.

WHAT IT MUST DO:
  · Accept encoded H.264 + AAC chunks over the E4 contract; mux FLV; publish RTMP(S).
  · RECONNECT. A venue uplink drops. Reconnect with backoff and resume, and report state
    to the ingest-health surface LS4 shipped — never to a log alone.
  · Never hold the stream key in a log line, an error string, or a panic message (see E6).

🚫 DO NOT reach for the stream key yourself — E6 owns how it arrives. Take it as a parameter
and assume it is secret.

GUARD: FLV framing against a known-good fixture; reconnect resumes rather than restarting the
broadcast; a key passed in never appears in any emitted string. MUTATION-TEST. Full suites.
