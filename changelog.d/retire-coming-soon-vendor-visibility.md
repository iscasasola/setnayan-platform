...
**⚠ Step 4 of the migration is what stops step 2 breaking signup — and CI caught it, not me.** `guard_vendor_profiles_entitlement` (20271004444950) pins the trust columns on its INSERT branch to the column defaults **verbatim**, so a self-created profile can never arrive pre-verified or pre-visible. It hardcoded `'coming_soon'`. Moving the default to `hidden` without moving that literal made every ordinary `INSERT INTO vendor_profiles (user_id)` — the signup trigger *and* `/open-shop` — raise `42501`: **no vendor could register at all.** Caught by `tests/db/vendor-addon-selfgrant-guard.db.test.ts`, whose own comment had predicted precisely this ("Getting either wrong would reject ordinary vendor registration"). The default and the guard now move together in one migration. Two companion test updates: the exposure-surface baseline (regenerated — the one-line diff is a genuine *narrowing*, from `is_published = true` to the explicit two-column predicate), and the registration assertion that hardcoded the old default.
