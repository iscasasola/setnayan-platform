...
- **Keyboard docking preserved + tightened** — when an input is focused, the sheet still docks above the keyboard (`bottom: kbInset`) with the inputs rendered last + `justify-end`, so the lowest text field sits flush against the keyboard. (Add form keeps `h-full justify-end` only while `kbOpen`.)
