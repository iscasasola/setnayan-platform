    ...
    <div className="space-y-3">
            ...
            >
              ...
              {i > 0 ? (
                <span
                  ...
                  className={`absolute left-[-50%] top-[23px] h-0.5 w-full ${
                    reached ? 'bg-terracotta/40' : 'bg-ink/10'
                  }`}
                />
              ) : null}
              {s.key === currentKey ? (
                <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 whitespace-nowrap text-[9.5px] font-bold uppercase tracking-[0.12em] text-mulberry">
                  You are here
                </span>
              ) : null}
              <span
                className={`relative z-[2] mx-auto flex h-[46px] w-[46px] items-center justify-center rounded-full border-2 bg-cream transition-transform ${
                  selected
                    ? 'scale-110 border-mulberry shadow-lg shadow-mulberry/20'
                    : aiActive
              ...
              >
                {s.pct >= 100 ? (
                  ...
                  <ProgressRing pct={s.pct} size={34} stroke={4} />
                )}
              </span>
      ...
      <div
        ...
        className={`rounded-2xl border p-5 sm:p-6 ${
          aiActive
            ? 'border-terracotta/40 bg-cream shadow-[0_2px_6px_rgba(92,37,66,0.07),0_16px_40px_rgba(92,37,66,0.11)]'
            : 'border-ink/10 bg-cream shadow-sm'
        }`}
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h3 className="m-serif text-xl text-ink">
            {active.label}
            <span className="ml-2.5 text-xs font-medium text-ink/45">
              {active.pct}% complete
            </span>
          </h3>
          <div className="flex gap-1.5">
            <button
              ...
              onClick={() => step(-1)}
              className="flex h-8 w-8 items-center justify-center rounded-full border border-ink/15 text-ink/60 transition-colors hover:bg-mulberry/10 hover:text-mulberry"
            >
              ←
            </button>
            <button
        ...
        {aiActive && active.aiNote ? (
          <div className="mt-4 flex items-start gap-2.5 rounded-xl border border-mulberry/20 bg-mulberry/5 px-3.5 py-2.5 text-[13.5px] text-ink">
            <span aria-hidden className="mt-0.5 flex-none text-mulberry">
              ✦
            </span>
