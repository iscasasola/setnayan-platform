...
appear in any public copy:
  ...
  · ⚠ NO LONGER A PROHIBITION — per-guest photo limits SHIPPED AND ARE SOLD.
    `lib/papic-guest-allotments.ts` is on main and the database enforces the split at capture:
    named guests get a set number each, everyone else splits the remainder equally, with a
    button and an automatic late release freeing the spare. PR #5024 landed the promotion-page
    claim, and P0-b measured `NEXT_PUBLIC_PAPIC_GUEST_BUY = true` in Vercel Production, so the
    guest-buy flow is LIVE. You MAY describe both. Read /papic for the shipped wording rather
