# E8 — the encoder can be fixed without asking couples to reinstall

**Model:** Sonnet 5 · **Effort:** high · **Wave:** after E7
...
TASK — E8: an auto-updater. NOT in the original 19–28 day estimate, and not optional.

🔑 WHY THIS IS NOT POLISH. `src-tauri/shell/index.html` is a meta-refresh to
https://setnayan.com — the app LOADS the live site rather than bundling it. So every WEB change
you deploy reaches users instantly, and the desktop app has needed no attention since June.

The encoder does not live in that half. It lives in the NATIVE half, which is FROZEN AT
WHATEVER VERSION SOMEBODY INSTALLED. Without an updater, an encoder bug means asking couples to
download a new build — in their wedding week, on the one date that cannot move.

BUILD: Tauri's updater, signed release artefacts, an update endpoint, and a check on launch.

⚠ AND IT MUST NOT UPDATE MID-BROADCAST. An update that restarts the app while a ceremony is on
air is a worse failure than the bug it fixes. Check on launch; if a broadcast is live, defer and
say so quietly. Never restart under the operator.

⚠ THE UPDATE MUST BE SIGNED. An unsigned update channel is a remote-code-execution path into
every couple's laptop. Tauri's updater expects a keypair — the private key is a repo secret and
must never enter the bundle.

GUARD: no update applies while a broadcast is live; the updater verifies a signature;
the private key is not in the built artefact. MUTATION-TEST each. Full suites.
COMMIT BEFORE A LONG VERIFY.
