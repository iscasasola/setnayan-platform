...
Three closures on `event_vendors`, plus one dead `anon` grant. Scoped by a 10-agent scope-then-attack run — the scope of what is **deliberately excluded** is the more important half.
...
**3 · One dead `anon` EXECUTE grant**, on a SECURITY DEFINER function that writes a confirmed booking status. `acquire_service_time_slot` is not reachable by anon in practice (it gates on `current_couple_event_ids()`, empty for a NULL `auth.uid()`), so this removes dead surface rather than closing a breach — but *"the gate happens to save us"* is weaker than *"anon cannot call it"* where a bug mints a booking. **Anon-callable SECDEF register: 217 → 216.**

⚠ **A first draft of this migration also revoked `vendor_claim_locked_qr`, and CI stopped it.** A prior security sweep had classified that function, flagged it, then **refuted its own flag and kept the grant deliberately** — *"a guest has no session; the token IS the credential. Revoking those would have broken the guest surface to fix nothing"* — and pinned the decision with a test that fails saying exactly that. The migration now carries an **inverse post-condition** asserting the grant is still there. **Asserting what must NOT change turned out to matter as much as asserting what must**, and a guard that makes a considered decision un-reversible by a later sweep is the reason that test exists.
