...
|---|---|---|---|
| W1 | **"0% commission" is claimed in five places on public pages** — "Verified suppliers · 0% commission" and "no commission on your suppliers, ever" on the home page, "**No commission on your bookings, ever**" on the *Open your shop* card, and twice more on `/features`. Suppliers are charged **5% of the booking** to ₱100,000, then 1%. The couple-facing reading is defensible (Setnayan never touches the couple's payment); the **supplier-facing** line is not. ⛔ **THE REST OF THIS ROW WAS FALSE AND IS CORRECTED 2026-09-22.** It read: *"`/pricing` is entirely couple-facing — there is no supplier pricing page, so the fee is disclosed nowhere public. A supplier learns it exists when the bill arrives."* **`/vendors` IS the supplier pricing page** — `vendor-tier-matrix.tsx` renders `{m.price}` from `getVendorPrices()` against the live `vendor_billing_catalog`, it carries the fee schedule and the first-5-free rule, and it is linked from the front door ("For suppliers"), the footer, `/features` and `/waitlist`. The fee **is** disclosed publicly. ⚠ **What IS true is smaller and is discoverability:** `/pricing` computes `vendorSubs` and uses it in exactly one place — the **JSON-LD** — so it publishes supplier prices to search engines and renders zero of them to a person. Whether it should link to `/vendors` is an open owner call. 🔑 **A session repeated this row without measuring it, and put the false claim into a PR and the corpus before catching it.** Re-measure: `grep -n 'm.price' apps/web/app/vendors/_components/vendor-tier-matrix.tsx` · `grep -n vendorSubs 'apps/web/app/(shell)/pricing/page.tsx'`. ✅ **DECIDED 2026-09-22: the 0% claim STAYS, with the /pricing second sentence propagated everywhere supplier-facing** — built, see `lib/commission-promise.ts`. | `/` · `/features` · `/vendors` · `/pricing` | ✅ **DECIDED** — built 2026-09-22 |
...
| ~~**Is "0% commission" the promise you want to keep?**~~ ✅ **ANSWERED 2026-09-22: IT STAYS**, with the `/pricing` second sentence propagated to every supplier-facing surface — built, `lib/commission-promise.ts`. ⛔ **This row also carried the false "no supplier pricing page exists" claim: `/vendors` IS one** (see the corrected W1). What was genuinely wrong was worse than the row said — three supplier pages still promised "0% commission **while we launch**", ungated, after production had collected ₱837.50. | Answered; nothing here blocks | §4b W1 (corrected) |
