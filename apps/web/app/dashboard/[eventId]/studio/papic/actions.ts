...
/**
 * Per-call sanity bound on a free Unli provisioning request. The "Unlock all of
 * Papic" Unli allowance is UNCAPPED (owner 2026-06-26), so this is NOT an
 * entitlement cap — just a fat-finger guard on a single submit; the couple can
 * provision again to add more. 250 mirrors the per-event guest QR ceiling.
 */
...
/**
 * Provision FREE Unli cameras for a couple who owns "Unlock all of Papic".
 * Completes the bundle's deferred camera grant (owner-locked 2026-06-26):
 * PAPIC_UNLOCK grants free, uncapped Unli cameras — so this mints Unli-tier
 * seats with NO order + NO payment step. The capture gate lets them shoot
 * because the event owns PAPIC_UNLOCK (papicUnliUnlockAllActive); we also stamp
 * paid_order_id to the (paid) PAPIC_UNLOCK order so the seats are traceable and
 * the order-paid gate passes too (belt + suspenders — refunding the bundle
 * re-locks both ways).
 *
...
export async function provisionUnlockUnliCameras(formData: FormData) {
  ...
  // Entitlement gate — must own (admin-approved) Unlock all of Papic.
  // eventSkuActive is bundle-aware + refund-aware; PAPIC_UNLOCK is a direct
  // package SKU (not a child of any bundle), so this is the authoritative check.
