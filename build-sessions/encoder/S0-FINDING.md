...
| `s0-ipc-realorigin-loopback-attempt1-no-redirect.log` | `SETNAYAN_PROBE_TOP=1 src-tauri/probe/run.sh ipc 1 …` on a quiet machine (load 5.96 at launch) — the webview loaded the bundled shell and **never navigated to setnayan.com**; the runner's 300-s silence cut-off stopped it. Kept as evidence; measures nothing. | `21d1eee84` | 15:27 |
| `s0-encode.log` | `SETNAYAN_PROBE_TOP=1 src-tauri/probe/run.sh encode 60 src-tauri/probe/s0-encode.log` — launched detached via `probe/detach.py` (`uptime` before: load 3.20 / 6.29 / 16.77) | `21d1eee84` | 15:35 |
...
### 4.1 Attempt 1 — `s0-encode-attempt1-hidden-then-suspended.log` (15:35:24 → 15:51:54, harness `21d1eee84`) — **10.83 minutes of a 60-minute run; the remaining 49 minutes are LEFT UNDONE** (a rerun is § 4.2)

Command: `SETNAYAN_PROBE_TOP=1 src-tauri/probe/run.sh encode 60 src-tauri/probe/s0-encode.log`, launched detached (`probe/detach.py`) on a quiet machine — `uptime` immediately before: **load 3.20 / 6.29 / 16.77**; the runner's `load-at-start` line reads the same. Page-load on `https://www.setnayan.com/login` at 15:35:29, `loopback listener 127.0.0.1:60873`, `window pinned always-on-top 560x360`, eval dispatched, 20-s settle, encode `t0 ≈ 15:35:50`. The page stayed on `/login` for the whole run (no `/dashboard` redirect this time — the totals below are one worker instance, uninterrupted). Every record arrived `via=postMessage headers=0` (the § 3.1 transport, as expected from this origin). `uptime` at the end: load 19.67 / 9.37 / 10.01 — the tail of the run overlapped another session's test burst, but by then the page had already been suspended (below), so no number in this section was taken under that load except the last two `[sample]` blocks.

`encode-config` (stage at 20.2 s): `"supported":true` with the config echoed intact — `codec:"avc1.42E01F", 1280×720, bitrate:2500000, framerate:30, bitrateMode:"constant", latencyMode:"realtime", hardwareAcceleration:"prefer-hardware", avc:{format:"annexb"}`, `minutes:60`. Frames were built with `new VideoFrame(canvas)` directly (`frameCtor:"canvas"` throughout — the ImageBitmap fallback never engaged).

**Every 60th second of the run** (`encode-10s` reports; `window` = the last 10 s, `totals` = since t0):

| min | visibility | win fps | chunks/s | kbps | late | skipped | drops | maxQueue | tot frames | tot chunks | pending | encodeQueueSize | maxQueueEver | keyframes | tot skipped | bp drops | errors | avg kbps |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 0.17 | visible | 30.09 | 29.99 | 2327.5 | 2 | 0 | 0 | 0 | 301 | 300 | 1 | 0 | 0 | 5 | 0 | 0 | 0 | 2327.8 |
| 1.17 | visible | 30.02 | 30.02 | 2308.6 | 0 | 0 | 0 | 0 | 2101 | 2100 | 1 | 0 | 0 | 35 | 0 | 0 | 0 | 2308.2 |
| 2.17 | visible | 30.01 | 30.01 | 2354.9 | 0 | 0 | 0 | 0 | 3901 | 3900 | 1 | 0 | 0 | 65 | 0 | 0 | 0 | 2326.8 |
| 3.17 | **hidden** | 15.82 | 15.82 | 2121.9 | 18 | 142 | 0 | 0 | 5443 | 5442 | 1 | 0 | 0 | 91 | 259 | 0 | 0 | 2312.9 |
| 4.17 | hidden | 17.16 | 17.16 | 2166.1 | 30 | 129 | 0 | 0 | 6424 | 6423 | 1 | 0 | 0 | 108 | 1078 | 0 | 0 | 2273.8 |
| 5.17 | hidden | 17.68 | 17.68 | 2179.2 | 32 | 123 | 0 | 0 | 7439 | 7438 | 1 | 0 | 0 | 124 | 1862 | 0 | 0 | 2248.8 |
| 6.17 | hidden | 17.61 | 17.61 | 2099.8 | 26 | 124 | 0 | 0 | 8486 | 8485 | 1 | 0 | 0 | 142 | 2615 | 0 | 0 | 2231.1 |
| 7.17 | hidden | 15.91 | 15.91 | 2113.2 | 28 | 141 | 0 | 0 | 9481 | 9480 | 1 | 0 | 0 | 158 | 3420 | 0 | 0 | 2216.4 |
| 8.17 | hidden | 16.28 | 16.28 | 2076.7 | 31 | 138 | 0 | 0 | 10507 | 10506 | 1 | 0 | 0 | 176 | 4195 | 0 | 0 | 2203.7 |
| 9.17 | hidden | 17.69 | 17.69 | 2146.0 | 37 | 122 | 0 | 0 | 11541 | 11540 | 1 | 0 | 0 | 193 | 4960 | 0 | 0 | 2197.0 |
| 10.17 | hidden | 16.82 | 16.82 | 2119.4 | 39 | 132 | 0 | 0 | 12489 | 12488 | 1 | 0 | 0 | 209 | 5812 | 0 | 0 | 2184.8 |
| 10.83 (last) | hidden | 17.01 | 17.01 | 2151.8 | 41 | 130 | 0 | 0 | 13178 | 13176 | 2 | 0 | 0 | 220 | 6323 | 0 | 0 | 2182.1 |

Across all 65 windows: fps min / mean / max **12.08 / 20.28 / 30.09**; windows with `drops` > 0: **0**; window `maxQueue` max: **0**; `encodeQueueSize` at every report: **0**; `pendingChunks` (frames handed in minus chunks out) never above **2**; `errors` **0**; keyframes exactly every 60 frames (5 per 10-s window while visible). The run has **two regimes and then a stop**, and the log plus the unified system log (`S0-logs/s0-encode-attempt1-unified-log-excerpt.txt`, extracted with `/usr/bin/log show` for pids 16718 / 16745 / 16741) say exactly where the boundaries are:

**Regime A — page visible, t0 → 170 s (17 windows).** **30.00–30.09 fps held**, chunks/s = fps, **2.31–2.36 Mbps** against the 2.5 Mbps CBR target, `late` 0–2, `skipped` 0, drops 0, queue 0. Process CPU from the `[sample]` lines: WebContent (16745) **~5 %** (max 7.1 %), WebKit GPU process (16741) **14–17 %**, the Tauri app itself 0.1 %. The GPU process is where WebKit hosts VideoToolbox, and the unified log has it logging, once per second, `com.apple.WebKit.GPU … (VideoProcessing) VCPRateControlSession (1280x720): input_fps=30.00, enc_fps=30.00, tx_fps=30.00` — VideoToolbox's rate-control session for a 1280×720 stream. **Because `require-hardware` is not a spec value (§ 8a), hardware use cannot be asserted through the API and has to be inferred**: from throughput at ≤ 7 % of one core in the page process, from thermals (below), and — stronger than either — from that `VCPRateControlSession` line, which is the hardware encode pipeline's own log, not a proxy.

**Regime B — page hidden, 171 s → 650 s (48 windows).** At **15:38:40.806** the unified log records `WebPageProxy::updateActivityState: view visibility state changed 1 -> 0` for the page (the pinned window was occluded — the owner was at the machine: `PowerChime … DisplayWake "com.apple.chimeOnAttach"` at 15:43:56 is a power-adapter attach; `displaysleep 0`, so the display did not sleep), followed within the same second by `WebProcessProxy::didChangeThrottleState: type=1` (Background) and `runningboardd … 16745 Set Darwin GPU to "Denys"`. VideoToolbox's `input_fps` went **30.00 → 26.00 → 16.72 → 12.43 within three seconds** and averaged **15.97** over the 509 hidden seconds. The harness clock in the worker fell to 15.8–17.7 ticks/s, `skipped` 122–142 per window, `late` 18–43 — and **the encoder stayed at `encodeQueueSize` 0, `maxQueueEver` 0, `backpressureDrops` 0** throughout: every frame it was handed came out as a chunk with at most two outstanding. The halving is upstream of `VideoEncoder` (a background-throttled process with GPU access denied, constructing `VideoFrame`s from an `OffscreenCanvas`), not encoder capacity. The proof of that is in the same log: every 10 s the postMessage IPC *response* (Tauri delivers invoke results by evaluating a script into the page — `WebPage::runJavaScriptInFrameInScriptWorld`) took a one-second foreground activity (`type=2`, `Darwin GPU "UserInteractives"`), and in each of those seconds `input_fps` snapped back to **29–30** (43 such seconds), then fell again when the assertion was released. CBR did what CBR does with half the frames: the bitrate held **2.08–2.18 Mbps**, so bytes per frame roughly doubled.

**The stop — suspension, not a crash.** At **15:46:40.812** `WebProcess::prepareToSuspend`, at **15:46:44.020** `WebProcessProxy::didChangeThrottleState(Suspended)`: WebKit suspended the hidden page's WebContent process after ~8 minutes in the background state. No further `encode-10s` record was produced; the app process stayed alive with nothing to say, and `run.sh`'s 300-s silence cut-off stopped it at 15:51:54. No crash report was written, memory pressure was not critical (`system vm pressure critical: 0`), and the encoder's last known state was `configured` with queue 0. Two things in that window are open: WebContent's footprint grew from **439 MB (15:38:56) to 910 MB (15:46:26)** during the hidden phase — attributing that (harness `VideoFrame`/canvas churn under the GPU-denied path, or WebKit) is **LEFT UNDONE**; and the `[sample]` process list is `sort -k2 -nr | head -6`, so at 0.0 % CPU the probe's own processes lose the tie-sort to six other zero-CPU WebKit processes and vanish from the log — which reads like a death and was not one. Not fixed here (harness left as is).

**Thermal.** `pmset -g therm` at every 10-s sample (63 samples, 16.5 min): `No thermal warning level has been recorded | No performance warning level has been recorded | No CPU power status has been recorded`. `pmset -g thermlog` printed **nothing** for the whole run — no `CPU_Speed_Limit` or `CPU_Scheduler_Limit` change — so thermal state at 10 min is *nominal*; 30 and 60 min were not reached.

**What this says, beyond the S0 question.** A WebCodecs encode inside WKWebView runs only as long as the page is *visible*: an occluded window halves the pipeline within three seconds and is suspended outright after about eight minutes, regardless of what the encoder itself can do. That is a WebKit process-lifecycle property, not a WebCodecs one, and it is new — S0.md did not ask it. For S1/S2/S5/S9 it means the live-studio page cannot be minimised, covered by another Space or hidden behind a fullscreen app while on air, or the stream ends; either the operator's window stays visible by design (an always-on-top program monitor, or a foreground assertion the app holds while live) or the encode does not live in the webview. This is an owner-level consequence and is flagged, not decided, here.
...
**(c) None of this triggers the B-remux branch.** `S0.md` says the plan branches to B-remux at S3 only if Question 1 fails on the accepted floor — i.e. if `VideoEncoder`/`AudioEncoder` are missing or refuse H.264/AAC. That capability floor **held** (answer (a)): every WebCodecs global is present, the encode config is accepted, AAC-LC is present, and § 4 encodes for the length of the run. What failed is a *transport* assumption in S5 (`InvokeBody::Raw` from the https origin), which is not a Question-1 outcome and has no bearing on the B-remux trigger. S3 proceeds on the native path; only S5 is re-scoped.
