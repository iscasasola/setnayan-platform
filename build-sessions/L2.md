# L2 — the controller stops claiming states the data denies
...
TASK — L2: three controls that assert a state the database contradicts.

All three were observed live on 2026-09-01. None needs the transport to work, so this session is NOT
blocked on L1.

DEFECT 1 — 'END BROADCAST' offers to end something that was never created.
  Measured: `panood_manual_on_air_at = 2026-09-01 02:16:07Z` while `panood_broadcasts` = 0 and
  `panood_watch_url` IS NULL. The manual 'We're on air' switch flipped the transport into its
  broadcast-running presentation. A host pressed it believing they had gone live, then went looking
  for a watch link that could not exist.
  The manual switch is a DECLARATION for the control room's red lamp. It must never present as a
  broadcast. Keep the switch; fix what it claims.

DEFECT 2 — 'Camera connected' survives the camera's death.
  Measured: the card read 'Camera connected' with the seat's last heartbeat 140 SECONDS stale
  (interval is 15s), because the string comes from the server render plus the `status` column,
  and nothing rewrites status when the beats stop. The controller's own source already names this
  shape — 'a green Camera connected nobody ever came back to clear'.
  The honest status must be resolved against last_seen_at at READ time, and a stale seat must say so.

DEFECT 3 — nothing warns before YouTube's 12-hour archive cliff.
  YouTube auto-archives a live stream UNDER 12 hours; past that the recording may not be captured at
  all. A Filipino wedding day runs prep-to-reception well over 12 hours. Today the host gets no
  warning, and discovers afterwards that the whole day has no recording — the single worst outcome
  this product can produce.
  Warn in the controller as elapsed broadcast time passes ~11 hours, telling the host to end and
  start a fresh broadcast so the archive is safely captured. Splitting also reads better in the
  editorial than one twelve-hour block.

⛔ OUT OF SCOPE: lib/panood-webrtc.ts and anything about why media does not flow. That is L1.

PROOF REQUIRED: mutation-test each assertion and print the occurrence count before → after. A guard
that stays green when you break the thing it guards is not a guard.
