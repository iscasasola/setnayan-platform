...
const DELIBERATE_EXCLUSIONS: Record<string, string> = {
  ...
  event_vendor_deposit_refusals:
    'Setnayan’s referee history of a money dispute between a couple and a supplier — admin-only (RLS on, no session grant). Its three *_user_id columns are actor stamps (who refused, which admin ruled, who ended it), not subject data; exporting by them would hand one party the other’s dispute record and Setnayan’s working notes.',
