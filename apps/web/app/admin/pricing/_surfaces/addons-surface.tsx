// Money-split studio surface (Catalog · addons tab) — the body of the former
// /admin/addons page, re-homed 2026-07-10. Collapsed to the Customer catalog
// (the Vendor sub-tab was V1-dead) to free the ?tab= param for the studio.
// The pricing-report download stays at /admin/addons/pricing-report.
/**
...
export async function AddonsSurface({ searchParams }: Props) {
  await requireAdmin();
  ...
  const visibleRows = customerRows; // vendor sub-tab was V1-dead; collapsed on the Catalog-studio fold
...
function CardLink({
  ...
  const href = isSelected ? '/admin/pricing?tab=addons' : `/admin/pricing?tab=addons&sku=${encodeURIComponent(row.sku_code)}`;
