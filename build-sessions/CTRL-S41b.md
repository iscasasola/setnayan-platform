# S41b · SILENT FAILURES, COUPLE-FACING tier (182 sites): a failed read must never look like "none" to a couple
...
1. S41 already finished the MONEY tier (64/69; the 5 left are booking-fee, owned by S6/S34) and the BOOKING tier (80/80) in PRs #5650 #5652–#5654 #5656–#5660. **Copy their pattern exactly**: read #5650 and #5656 first. Now do the **COUPLE-FACING tier (182 sites)**, as reported by the scanner in `lib/ugat/both-ends.ts` / `lib/supabase-unread-error-scan.ts` (PR #5625).
...
3. S41 left four known on-screen cases for later. **Include them**: the couple's "What's next" drops a source that failed to load, and the checklist budget-health widget hides. (The supplier pending-inquiry badge reading 0 and the Overview feed dropping disputed handovers belong to the supplier tier and will come next.)
...
5. Stop when the couple tier is done, or after 8 PRs, and report the counts left.
...
- **Your final message must end with:** `CONTROLLER ▸ S41b · <DONE|BLOCKED|NEEDS-OWNER> · PR #<n> <state> · <one line>`
