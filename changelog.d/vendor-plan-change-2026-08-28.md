...
### Credit EXPIRES when a shop lapses (owner 2026-08-28) — and never silently

Second-day ruling: *"it expires when they lapse"*, chosen over keeping it. This **reverses** the default the first migration shipped, which persisted the balance because the question was open at the time. Not a correction of a mistake — a decision that had not been made yet.

**The moment of expiry is the SWEEP, not the clock.** `tier_expires_at` passing does nothing on its own; `sweep_vendor_tier_expiry` is login-driven and cron-free and is the only thing that actually runs. ⚠ **So a shop whose owner never signs in keeps its balance until somebody does** — a real consequence of a cron-free design, stated rather than hidden. Attaching expiry to the clock instead would make the money "gone" at a moment no code observes, so two readers would disagree about the balance depending on whether anything had swept, and the screen would be one of them. A test asserts the balance is still there *before* the sweep runs, so the moment is part of the contract rather than an accident of when the test happened to look.

🚨 **A lapse and a plan change are not the same event, and that is the dangerous part.** The applier and the lapse live in the same function and are reached by the same condition — `tier_expires_at` in the past. Only one is a shop going away: a shop moving onto the Solo plan it scheduled **and paid for** is *continuing*, and taking its balance there would be theft dressed as policy. The applier returns before the lapse, `subscription_credit_php` is deliberately absent from its UPDATE, and **both directions are pinned** — a landing scheduled change keeps the balance and writes no `lapse` row; an unpaid schedule lapses and loses it.

**Never destroy a balance silently.** New `vendor_credit_ledger` records every movement with the amount, the balances either side, and the reason. 🔑 **Written by a TRIGGER ON THE COLUMN, not by the sweep** — a ledger written by the paths somebody remembered is a ledger that misses the next path. The expiry, the activation assignment, the cancellation refund and anything added later all move through that column and are all recorded. The reason comes from a session setting and falls back to `unspecified` rather than failing: an unexplained movement is far better than an unrecorded one. Non-movements are skipped, so a zero balance lapsing writes no noise.

**The test was INVERTED, not replaced,** and confirmed **red against the pre-change code** before the migration existed. Deleting a test and adding another is how a behaviour ends up unpinned in the gap between them.

🚨 **The exposure freeze earned its keep again: the new table was born open.** Supabase's default privileges hand `anon` and `authenticated` full SELECT/INSERT/UPDATE/DELETE on a new public table, and `REVOKE ... FROM PUBLIC` does **not** remove a grant made to a role directly — so the first cut had `anon` holding SIUD on a log of shop money movements. RLS would still have filtered the rows, but table privileges are the only defence in depth behind RLS and a money log is the last place to lean on a single layer. Revoked from both roles by name: `anon` gets nothing, `authenticated` gets SELECT only.

⚠ **Three stale comments were corrected in the same commit rather than left to rot.** The first migration described the balance as *"NOT cleared when a plan lapses"* in a column comment, a function comment and a prose block. A column comment is **not** overwritten by a later migration, so that one would have been a live falsehood in production; the new migration therefore restates it authoritatively too, because **a reader queries the object, not the file**.

### Downgrade billing — CONFIRMED by the owner, not assumed

The gap I filled by judgement is now ruled on: **pay now, starts at term end**, exactly as built. Recorded as owner-confirmed 2026-08-28 rather than left standing as an assumption.

### Open, and flagged rather than decided

✅ ~~What happens to carried credit if a shop lapses entirely.~~ **RULED 2026-08-28: it expires.** Built, with a ledger row behind every disappearance. The persist behaviour and its test are gone — inverted, not deleted.

⚖ **How much money that ruling moves, measured rather than assumed.** The term rule was expected to shrink the maximum credit "from months to weeks". Half of that is right and half is not, so both halves are recorded — and the second half is what makes the expiry ruling consequential rather than cosmetic:
...
- **From a cancelled scheduled change: one plan term's price, and that is not weeks.** A shop can pay for a same-length downgrade, then call it off, and what they paid becomes credit — realistically ₱10,400 (Solo annual), up to ₱104,000 in the Enterprise-annual edge. That is money they genuinely handed over and have had no service for, so it has to go somewhere; returning it as credit is right. **This is why the expiry ruling matters:** the balance that now disappears on a lapse can be a full year's payment, not a rounding detail — which is exactly why it disappears into a ledger row rather than into nothing.

🔴 **NOT BUILT, and reported rather than assumed: telling the supplier.** A balance vanishing unannounced reads as theft even when it is policy. It is **not free** — `notification_type` is a Postgres enum with 76 labels, so a new kind of notice needs its own migration plus a decision on whether it also goes to email and push. (This codebase has already shipped three notification types the database never had; they were refused silently.) The pieces all exist and the wiring is small, but it is a build with an owner decision in it — *should a shop be warned before the money goes, told after, or neither* — so it was left alone.
