...
| **No limit on customers per date** | **+₱2,500** ✅ (owner 2026-08-29) | Removes **BOTH** per-date ceilings: customers a shop may be **chasing** for one date (accepted-but-not-yet-locked · base 10) **and** couples it may **pick off the booked-out waitlist** for a date already sold (base 5). Asked what going past 10 should cost, the owner answered ***"2500 for no limit"***; asked whether the waitlist was included, ***"yes wait list add them"*** — one axis, both ceilings. Enforced in the DATABASE (`vendor_pipeline_is_unlimited` → `enforce_vendor_whitelist_per_date` · `clamp_vendor_waitlist_to_tier` · `pickWaitlistCouple`), not only in the caps overlay, and both on-screen countdowns stop naming a limit for a shop that holds it. ⛔ **`vendor_profiles_max_waitlist_0_10` is deliberately NOT widened**: under "no limit" the stored number stops being a ceiling at all, so raising it to 999 would leave a bigger number in a column that nothing enforces — *a ceiling nobody enforces reads as a promise*. ⛔ **The axis raises a ceiling; it never conjures a waiting list onto a plan that has none** — the zero-allowance arm is checked first |
