-- save_vendor_service_add_coverage_id
-- Created via `pnpm migration:new`. Prefix auto-allocated to sort AFTER every
-- existing migration. Idempotent (CREATE OR REPLACE FUNCTION).
--
-- Wire `coverage_id` (added by 20270426250948) into the atomic wizard writer so
-- the guided "create a service" flow can link a card to its coverage — parity
-- with createVendorService/updateVendorService. Re-defines the LATEST copy
-- (20270428382392, which added base_pax) verbatim + coverage_id in the INSERT
-- column list, the VALUES, and the UPDATE SET. Ownership of the coverage is
-- validated app-side before this RPC is called. Everything else is unchanged.
