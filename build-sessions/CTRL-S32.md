# S32 · Day-of screens: the emcee's pronunciation guide and the venue screen
...
- **DAY-7:** a pronunciation / emcee-question mechanism, so the host or emcee has the couple's names and key names spelled phonetically, collected from the couple. Find the design in the corpus first.
- **DAY-12:** `venueScreen` under `app/panood` (zero hits). The display mode for a venue's big screen. Reuse the panood/Live Studio renderers. Remember LS6 deleted the broadcast-day model on purpose, so don't resurrect it (`git log --diff-filter=D`).
If either has no design in the corpus, stop and report rather than inventing it.
...
- **Your final message must end with:** `CONTROLLER ▸ S32 · <DONE|BLOCKED|NEEDS-OWNER> · PR #<n> <state> · <one line>`
