-- public_venue_scene v10 — carry the vendor's per-event booth POSTER.
--
-- Owner directive 2026-07-21: a booked vendor uploads a design FOR that event;
-- their account logo shows alongside it. v8 lit up the logo (tier), v9 the card
-- items; this adds the poster so the guest walk renders the whole booth.
--
-- poster_ref is the RAW stored ref (r2://bucket/key), exactly like vp.logo_url
-- beside it. The READER resolves it: /[slug]/venue/page.tsx and
-- /api/venue-scene both already batch-resolve booth logo refs through
-- displayUrlForStoredAsset and now do the same for posters. Nothing is
-- presigned inside the RPC.
--
-- Joined on (event_id, vendor_profile_id) — NOT on the event_vendors row —
-- because there is ONE artwork per vendor per event: a vendor booked for two
-- services shows the same poster on their booth. That is the cardinality
-- event_vendor_booth_posters' UNIQUE constraint already states.
--
-- Gating stays CLIENT-SIDE and unchanged: boothCanBrand(tier) governs whether
-- the branded backdrop and the poster render at all, so a solo/verified booth
-- receives the ref but renders generic — identical to how logoUrl already
-- behaves. No new tier logic in SQL.
--
-- Idempotent CREATE OR REPLACE. IDENTICAL to v9 EXCEPT the added 'posterUrl'
-- key and the one LEFT JOIN feeding it. Signature unchanged (text, text);
-- STABLE SECURITY DEFINER + search_path + every v6-v9 join preserved.
...
BEGIN
  ...
  -- The vendor's own design FOR THIS EVENT. One artwork per (event, vendor),
  -- so this joins on the vendor profile, not on the event_vendors row — a
  -- vendor booked for two services shows the same poster on their booth.
