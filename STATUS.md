...
| **Monogram tier system + AI-automated Bespoke flow (queued for new session)** | Owner locked the 3-tier model + Bespoke pricing + automation flavor 2026-05-14 (reference images shared). **Free / Basic**: 2 letters + `&` (e.g. `J & S`), 1-2 default fonts, simple geometric frame — presentable but not custom. **Pro (₱99 widget upgrade per 0004)**: 2 letters OR full first names (e.g. `JAMES & SAVA`), ornamental + heritage frames (laurel wreath / royal crest / sampaguita / capiz / banner-ribbon), 8+ premium fonts, placement variants. **Bespoke (₱2,999, NEW SKU)**: AI-generated interlocked letterforms via DALL-E 3 HD + vectorizer.ai semi-automated flow. Replaces the existing flat `Custom Monogram Pack — remove watermark` SKU. **AI flow**: couple submits brief (3 personality words + initials/names + motif preference) + pays ₱2,999 → server fires DALL-E 3 × 6-8 HD variations with templated prompt → PNGs stash to R2 → admin queue at `/admin/bespoke-monograms/[orderId]` for designer pick top 2 (~5 min) → auto-vectorize via vectorizer.ai → couple picks final at `/dashboard/[eventId]/bespoke-monogram/pick` OR requests 1 free revision → final SVG goes live event-wide. **Cost per piece**: ~₱54 DALL-E HD + ~₱15 vectorize + 10 min designer = ~₱200 all-in → ~93% margin at ₱2,999. **Owner signups needed**: OpenAI Platform (`OPENAI_API_KEY`), Vectorizer.ai (`VECTORIZER_API_KEY`). Full-auto (no designer step) is V2 once prompt template proves 90%+ usable | 4-5 days code once API keys are in Vercel |
