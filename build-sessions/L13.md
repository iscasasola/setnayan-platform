# L13 — the program monitor shows what actually goes out

**Model:** Sonnet 5 · **Effort:** low · **Wave:** now

Measured against `origin/main` @ `c0ee239ac` on 2026-09-02. Re-fetch before you act.
...
TASK — L13: the operator directs on one framing and broadcasts another.

THE DEFECT, seen side by side by the owner on 2026-09-02 — the controller's CH 1 monitor and the
program output window, same camera, different frame:

    controller monitor   camera-feeds.tsx   ChannelVideo default   object-cover
    program output       program-surface.tsx                       object-contain

Both are DELIBERATE. The program surface says so: *"One full-bleed video. object-contain — never
crop the couple's frame."* That is a defensible broadcast choice; cropping a portrait phone shot to
16:9 throws away most of the subject.

🔑 THE PROBLEM IS NOT EITHER CHOICE, IT IS THAT THEY DISAGREE. A shot that looks composed on the
monitor goes out pillarboxed between black bars, and the operator cannot see it happening. A
program monitor that does not show the program is the one thing it exists to prevent.

SCOPE — narrow, and resist widening it:
  1. The controller's CH 1 PROGRAM MONITOR matches the program output's fit.
  2. The small camera TILES keep `object-cover`. They are thumbnails for picking a shot, not for
     composing one, and cropping is right there. ⛔ Do not change them.
  3. ⚠ `ChannelVideo` takes a `className` and the default is used in more than one place — grep
     every call site before changing the DEFAULT. Changing a shared default to fix one surface is
     how the tiles break silently. Prefer passing the fit explicitly at the program-monitor site.

ALSO — one copy line, because no code can fix it: those black bars appear because the phone is
shooting PORTRAIT. Hold the phone landscape and the frame fills. Every camera operator will be a
guest who does not know that. Add it to the camera join screen
(app/panood/cam/[token]/_components/panood-camera-publish.tsx) where the operator is already being
told to keep the screen open — one sentence, not a tutorial.

PROOF REQUIRED: describe the monitor and the program output for the SAME portrait source, before
and after. If they still differ, say so — matching them is the whole task and a near-miss is a
fail.
