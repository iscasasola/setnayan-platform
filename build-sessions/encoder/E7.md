# E7 — it installs on both platforms without frightening anybody

**Model:** Sonnet 5 · **Effort:** high · **Wave:** after a working binary
...
TASK — E7: cross-platform build, signing, notarization. Routinely underestimated; budget honestly.

STARTING STATE, verify it: `.github/workflows/build-desktop.yml` already builds BOTH legs —
macOS aarch64 `.dmg` and Windows `x86_64-pc-windows-msvc` `.msi`. It is `workflow_dispatch:`
only, deferred by the owner 2026-06-20 to stop a 10×-minutes macOS build running on every web
PR. It had not run since 2026-06-25 before E0.

  · macOS: Developer-ID signing + notarization when the `APPLE_*` secrets exist, with an
    ad-hoc fallback so CI never breaks. Confirm notarization actually completes — an ad-hoc
    binary still shows a Gatekeeper warning, and the fallback is silent about which path ran.
  · ⚠ WINDOWS IS UNSIGNED. The workflow says so: "Windows is still unsigned (Phase 2)". An
    unsigned `.msi` triggers SmartScreen — "Windows protected your PC" — which is the same
    category of friction as Google's unverified-app screen, landing on a couple in their
    wedding week. A code-signing certificate is a PURCHASE and an owner decision. SURFACE THE
    COST AND THE LEAD TIME; do not buy anything.
  · Re-enable an automatic trigger ONLY if the owner asks. The deferral was deliberate and the
    Actions-minutes reasoning has not changed.

🔑 A GREEN BUILD IS NOT AN INSTALLABLE APP. Download both artefacts and INSTALL THEM on a real
machine of each OS. Report what a first-time user actually sees, warning dialogs included.

GUARD: the workflow still produces both bundles; the notarization step is not silently
skipped. Full suites. COMMIT BEFORE A LONG VERIFY.
