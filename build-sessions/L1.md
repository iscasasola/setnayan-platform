# L1 — a camera's picture reaches the controller

**Model:** Opus 5 · **Effort:** high · **Wave:** now · **Blocks L3, L6, and Google verification**

Measured against `origin/main` @ `7d6206679` and the live database on 2026-09-01. Re-fetch and re-measure before you act.
...
TASK — L1: a camera's picture reaches the controller.

THE DEFECT: no camera has EVER put a picture on the Live Studio controller. Claim, binding and
heartbeat all work; media never arrives. `panood_broadcasts` and `live_studio_roam_streams` are
both 0 and always have been.

WHAT IS ALREADY RULED OUT — do not re-litigate these, re-measure only if you doubt them:
  · NOT the streaming flag. The monitor renders 'nothing joined on this channel yet', which is the
    streamingOn===true branch; the false branch reads 'preview — live video arrives with the
    streaming rollout'.
  · NOT RLS. `panood_rtc_can_access('panood-rtc:044f7e64-95aa-4dcb-84c1-7263bf494eaa')` returns
    TRUE for the couple's uid, tested by setting request.jwt.claims. The policies on
    realtime.messages exist, are scoped {authenticated}, and call the predicate correctly.
  · NOT the slot key. Both sides use cameraSlotForIndex → `cam{camera_index}`.
  · NOT camera liveness. Heartbeats measured at 1–5s while failing.
  · NOT background-tab throttling. Reproduced with both windows visible side by side.

THE ONE MEASUREMENT THAT MATTERS, AND IT IS ALREADY IN: the homepage/`/panood` two-phone demo
WORKS on the same devices and the same network where Live Studio fails. Its transport
(lib/demo-webrtc.ts) uses a PUBLIC Realtime channel; Live Studio (lib/panood-webrtc.ts) uses
`private: true`. getDemoIceServers and getPanoodIceServers are line-for-line identical — same STUN,
same mintTurnIceServers. So the network is exonerated and the private channel is the suspect.

⛔ DO NOT START WITH TURN. Re-run the A/B first (two-phone demo vs Live Studio, same devices). If
the demo connects, TURN cannot be the cause and a day was already lost to that theory.

⛔ DO NOT REBUILD THE TRANSPORT. It is a working design; find what refuses.

ALREADY SHIPPED TODAY, do not redo: PR #5057 primes `supabase.realtime.setAuth()` on both ends
before subscribe and reports CHANNEL_ERROR/TIMED_OUT/CLOSED instead of discarding them. It merged,
deployed, and the camera STILL failed — so setAuth alone is not sufficient, or a second fault sits
behind it. PR #5059 makes turnConfigured() visible in the controller and distinguishes a broken TURN
key from an absent one. READ WHAT THOSE TWO NOW REPORT before forming a theory.

SCOPE:
  1. Determine why the private channel does not carry media when the public one does.
  2. Fix it.
  3. Fix the copy: 'couldn't reach the controller on this network — try the same Wi-Fi as the
     operator' is shown on the pc-failed path, so an AUTHORIZATION failure is reported as a network
     fault. That sentence sent two sessions hunting AP isolation and TURN pricing. Whatever the
     cause turns out to be, the message must name it.

PROOF REQUIRED: a real second device publishing, and the controller's program monitor showing its
picture. A green state chip is not proof; neither is a heartbeat. Say what you saw.

If you cannot make it connect, that is still a complete session — report the narrowest true
statement about where it stops, with the command and output that showed it.
