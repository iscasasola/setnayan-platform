 ...
 * Non-redirecting (`useActionState`-shaped, the inline-docs-actions idiom):
 * the card saves optimistically and reverts + toasts on an error value.
 *
...
export type AutoReplySaveResult =
  | { ok: true; enabled: boolean; dailyReplyCap: number }
...
export async function updateAutoReplyConfig(
  ...
  // toggle never clobbers the cap (and vice versa). A fresh row picks up the
