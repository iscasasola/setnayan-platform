-- Papic "Unlock All" — lift the per-guest 150-credit cap (guest disposable
-- camera). The authoritative cap lives in papic_record_guest_capture; this
-- redefines it so an event holding an ACTIVE (paid/fulfilled) PAPIC_UNLOCK_ALL
-- order never exhausts a guest (v_unlimited). The match is paid/fulfilled ONLY,
-- mirroring the app-side eventHasPapicUnlockAll (checkOrderActive) — a pending
-- 'submitted' pass does NOT lift the cap (the handshake).
--
-- Everything else is BYTE-IDENTICAL to the 20270216612756 definition (clip
-- media + duration + poster + advisory lock + ownership gate). Additive +
-- idempotent (CREATE OR REPLACE), no new columns, no RLS change.
--
-- GRACEFUL: if this migration is NOT applied, the cap stays 150 and the feature
-- degrades to today's behavior — the app-side fetchGuestQuota lift is
-- independent and safe on its own (it only affects the display + the route
-- pre-check; this RPC is the hard gate).
...
BEGIN
  ...
  -- Papic Unlock All (the Papic-vertical everything-pass): an ACTIVE
  -- (paid/fulfilled) PAPIC_UNLOCK_ALL order lifts the per-guest 150-credit cap.
  -- Mirrors apps/web/lib/entitlements.ts eventHasPapicUnlockAll (active-only) —
  -- a pending pass never lifts the cap.
  SELECT EXISTS (
    SELECT 1
    FROM public.orders
    WHERE event_id = v_event_id
      AND service_key = 'PAPIC_UNLOCK_ALL'
  ...
  -- Unlock All events never exhaust; otherwise the 150-credit pool binds.
