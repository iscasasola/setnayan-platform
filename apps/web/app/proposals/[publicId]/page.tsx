...
export default async function ProposalDetailPage({ params, searchParams }: Props) {
  ...
  // Mark-on-VIEW: the couple opening a delivered quote flips it sent→viewed so the
  // vendor sees "your quote was opened". Fire off the request path; the RPC
  // re-verifies customer-side membership + only transitions sent→viewed. No-op for
  // the vendor's own preview or an already-viewed proposal. (The token-settle side
  // of this was retired with the lead-token HOLD path.)
  if (!isVendorSide && proposal.status === 'sent') {
    after(() => markProposalViewed(proposal.public_id, user.id));
  }
