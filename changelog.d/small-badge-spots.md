...
`app/v/[slug]/page.tsx` (the shop's own public page) is deliberately **not** touched here — a parallel session (HONEST SHOP, PR #5450) is editing that file, and its badge fix is PART 3 of this build (L2), after that PR merges.
...
Tests: `lib/the-badge-has-a-deadline.test.ts` (5 cases). Mutation-checked (reverted the vendors/page.tsx badge fix to raw `public_visibility === 'verified'`; both the "sites actually call hasVerifiedBadge" and the "billed at exact count" tests failed as expected; restored from an explicit backup, verified against the commit). Typecheck clean (empty log). Lint clean (no `Error:`).
