# L8 — the free path is not a footnote
...
TASK — L8: the doorway to a free product leads with a ₱3,000 button.

THE OBSERVATION (owner-visible, 2026-09-01): on
`/dashboard/<eventId>/studio/live-studio-control` — the page the sidebar sends every host to —
the primary control is a large **'Add Live Studio · ₱3,000'** button, and **'Open the controller —
go live free with one camera →'** is plain text beside it.

WHY THAT IS AT ODDS WITH A LOCKED DECISION. Wave 3, owner-locked 2026-07-25 § 4d, is 'REHEARSE FREE,
PAY TO BROADCAST'. Its own source says: a host WITHOUT Live Studio opens a fully working rehearsal
room, and 'seeing the cameras actually working IS the conversion mechanism'. The live /pricing page
promises single-camera livestream FREE. So the doorway to a free product currently reads as though
the product costs ₱3,000.

⛔ THIS IS NOT A LICENCE TO REDESIGN THE PAGE. RULE 0 applies: read what is there, change the
smallest thing that resolves the contradiction, and show the DELTA. Recreating a working screen is
a defect, not a deliverable.

SCOPE:
  1. For a host who does NOT own Live Studio, make opening the controller at least as prominent as
     buying. The purchase must stay clearly available — this is about order and weight, not hiding
     the price.
  2. For a host who DOES own it, opening the controller is the only sensible primary action;
     confirm that is already the case and say so rather than assuming.
  3. Leave the price, the SKU wiring and the buy drawer untouched.

⚠ SURFACE, DO NOT DECIDE. If making the free path primary would materially change how the SKU
sells, say so in the handback and let the owner rule. Emphasis on a paid product's doorway is a
commercial decision that happens to live in a component.

PROOF REQUIRED: state what the page renders in BOTH ownership states, before and after. A claim
about the un-owned state that was only observed in the owned one is not a measurement.
