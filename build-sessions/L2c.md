# L2c — a long wedding day does not lose its recording

**Model:** Sonnet 5 · **Effort:** medium · **Wave:** now · **The last unbuilt piece of L2**

Measured against `origin/main` @ `6267be4a8` on 2026-09-02. Re-fetch before you act.
...
TASK — L2c: warn the host before YouTube stops archiving.

⚠ TWO OF L2's THREE DEFECTS ARE ALREADY FIXED. Do not rebuild them, and do not trust any
description of them — measure.
  · #5062 (merged) — the End button no longer claims to end a broadcast that was never created.
  · #5073 (merged) — `lib/live-studio-channel-freshness.ts` resolves camera staleness at read time
    and the card now keeps up. It distinguishes a never-claimed seat from one that joined and
    stopped, and it refreshes on its own rather than freezing at page load.
  · THIS SESSION IS THE THIRD DEFECT ONLY. `decideArchiveGuard` is named once in #5062's changelog
    and defined nowhere.

THE DEFECT: YouTube auto-archives a live stream ONLY when it runs UNDER 12 HOURS. Past that the
recording may not be captured at all. A Filipino wedding day — preparation through reception — clears
12 hours routinely. Today the host gets no warning, keeps streaming, and discovers AFTERWARDS that
the entire day has no recording.

🔑 THIS IS THE WORST OUTCOME THIS PRODUCT CAN PRODUCE. Every other open defect degrades an
experience; this one loses a wedding, silently, unrecoverably, and the couple finds out later.
It also empties the editorial's 'Watch the Film' section, which is the thing they were sold.

SCOPE:
  1. A PURE decision function — take `now` and the broadcast's start time as PARAMETERS. Never call
     Date.now() inside it, or it cannot be unit-tested against an 11-hour boundary without faking a
     clock. Return a state, not a string: nothing / approaching / past.
  2. Surface it in the controller as elapsed time approaches ~11 hours: end this broadcast and start
     a fresh one so the archive is safely captured. Splitting also reads better in the editorial
     than one twelve-hour block — three films, not one.
  3. ⚠ A NOTICE, NEVER A BLOCK. Do not stop a host broadcasting because a timer says so. A wedding
     that is running long is not a wedding that should go dark.
  4. Say what the host loses, not what the system does. 'YouTube stops saving after 12 hours' is
     information; 'your recording will not be saved' is the sentence that makes someone act.

⛔ OUT OF SCOPE: the transport, the camera card, storage and R2. The download question is L4 and is
awaiting an owner ruling — do not pre-empt it, and do not promise a file this product may not keep.

PROOF REQUIRED: unit-test the boundary from both sides with an injected `now` — 10h59m quiet,
11h01m warning — and mutation-test it. Print the occurrence count before → after.
