...
- **`app/admin/pakanta/page.tsx` (new) — the back-office Pakanta queue.** Exactly what the 2026-06 schema anticipated ("Admins read all rows so the back-office Pakanta queue can scan for new intakes"). Lists `pakanta_intake_drafts`, joins `events` for `display_name` / `love_story` / `story_tone`, and renders each composed brief (purchased/pending float to top) with a copy-paste block for the music team. Server component, `createAdminClient()` (service role), layout-gated auth, `logQueryError` graceful-degrade — same shape as `/admin/account-deletions`. Added a **"Pakanta queue"** item to the admin sidebar Work group.

**Verification:** `pnpm typecheck` (2/2 packages) + `pnpm lint` (2/2, FULL TURBO clean) GREEN in a fresh worktree off origin/main. No migration (love_story is JSONB; the `pakanta_intake_drafts` admin-read RLS already exists). Composer is a pure function; admin page mirrors an existing pattern + degrades gracefully if the table is unmigrated.

**SPEC IMPACT:** Logged as a DECISION_LOG.md row (2026-06-13). Realizes iteration 0036's "the order flow can copy customer_brief from the matching draft" intent via `love_story` instead of a re-interview. **Phase 2 (separate PR, flagged to owner):** the couple-facing collection surface that writes `pakanta_intake_drafts` (the wizard intake that did this was deleted in #1320) — it should pre-fill the story from `love_story` and only ask the music top-up; plus optionally add a `pet_names` ("what you call each other") field to the onboarding love stage so it flows from one interview. Until then the queue lists existing/legacy drafts and composes from whatever is present.
