 ...
 * cards. The caller (app/page.tsx) resolves the (already double-gated) vendor
 * list via `fetchHomepageSpotlight` and passes it in. An empty list renders
 * NOTHING — the strip only appears once the owner has flipped
 ...
 *
 * 🪤 `logo_url` DOES NOT HOLD A URL. `vendor_profiles.logo_url` stores an
 * `r2://bucket/key` reference (the shop editor writes it that way), and a
 * browser cannot fetch that — it renders a broken-image glyph, throws nothing,
 * logs nothing. This strip passed the raw value into `<img src>`, so the very
 * first thing the homepage would show once the owner switched the strip on was
 * a broken picture next to every featured vendor's name.
 *
 * Note WHY it was still raw: the sibling builder `fetchVerifiedVendorPool` in
 * lib/spotlight-awards.ts resolves logos, but that is the RECOMPUTE path. The
 * homepage READ path (`fetchSpotlightAwards` → `fetchHomepageSpotlight`) selects
 * `logo_url` through the embedded vendor join and hands it over untouched. Two
 * builders in one file, only one of them fixed.
 */
 ...
 * a logo is decoration and the card already falls back to a Trophy tile, so a
 * signing hiccup must never take the public homepage down.
 */
...
export async function HomeSpotlightStrip({
  vendors,
}: {
  ...
  // Indexed by position: `logoUrls[i]` belongs to `vendors[i]`.
  const logoUrls = await Promise.all(vendors.map((v) => resolveDisplayUrl(v.logo_url)));
