...
const ERROR_COPY: Record<string, string> = {
  ...
  invalid_type:
    "That event type isn't available yet — pick one from the carousel to continue.",
...
export default async function CreateEventPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const launchStatus: LaunchStatusRow[] = (launchRows as LaunchStatusRow[] | null) ?? [
    ...
    { ceremony_type: 'jewish',    status: 'active' },
    { ceremony_type: 'born_again', status: 'active' },
  ];
    ...
    <div className="mx-auto w-full max-w-2xl px-4 py-10 sm:px-6 lg:px-8">
      <header className="mb-8 space-y-2">
        ...
        <p className="text-base text-ink/60">
          Swipe through and pick the kind of event you&apos;re planning.
        </p>
      </header>
