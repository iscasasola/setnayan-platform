# L9 — two loose ends from the Live Studio sweep

**Model:** Sonnet 5 · **Effort:** low · **Wave:** now

Measured against `origin/main` @ `6267be4a8` and the live database on 2026-09-02. Re-fetch before you act.
...
TASK — L9: two small, unrelated tidies. Both are measurements, not hunches.

⚠ IF EITHER IS ALREADY RESOLVED WHEN YOU MEASURE, SAY SO AND SKIP IT. Reporting 'not there' with the
command that showed it is a complete and successful outcome — do not invent adjacent work.

── 1 · THE ODD ANON GRANT ───────────────────────────────────────────────────
Measured 2026-09-01, after #5064 restored the RLS predicate grants:

    call_rtc_can_access               postgres | service_role | authenticated
    live_studio_guest_rtc_can_access  postgres | service_role | authenticated
    panood_rtc_can_access             postgres | anon | authenticated | service_role   ← extra

`panood_rtc_can_access` carries an explicit `anon` EXECUTE grant its two siblings do not.
NOT EXPLOITABLE — the function's first line is `IF auth.uid() IS NULL THEN RETURN FALSE`, and the
policies on `realtime.messages` are scoped `TO authenticated`, so the `anon` role never reaches
it through RLS. But #5064's own header states the intent — 'authenticated only, NOT anon, whose
revocation stands' — and this function contradicts it.
DECIDE AND SAY WHY: revoke it to match its siblings, or document why it must stay. An inconsistency
that nobody can explain is how the next security sweep breaks something.
⚠ A migration means the Ugat map must keep up — two required db-tests enforce it. Never weaken a
check to go green.

── 2 · THE SPF RECORD ───────────────────────────────────────────────────────
Measured 2026-09-01 with `dig +short TXT setnayan.com`:

    "v=spf1 include:icloud.com ~all"

iCloud is authorised. Resend is not — and per CLAUDE.md all transactional email goes through Resend
(email-only, no SMS in V1). If mail is sent from the apex domain it is failing SPF and heading for
spam, INCLUDING the payment notifications shipped in #4595 and every order receipt.
⚠ THIS MAY BE FINE. Resend is commonly configured on a subdomain with its own SPF and DKIM, which
the apex record would not show. MEASURE BEFORE CONCLUDING: check for a Resend subdomain
(`dig +short TXT send.setnayan.com` and similar), and check what `from` address the code actually
sends on — grep the Resend client for the sender domain. Report which it is.
⛔ DO NOT CHANGE DNS. GoDaddy is the registrar and DNS is an owner action. Your deliverable is a
measurement and a recommendation, not a record.

PROOF REQUIRED: the command and its output for each finding. If item 2 turns out to be correctly
configured, that is the most valuable possible result — it closes a live worry.
