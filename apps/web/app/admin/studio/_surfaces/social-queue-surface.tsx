 ...
 * re-homed byte-identical from app/admin/social-queue/page.tsx into the tabbed
 * /admin/studio studio (Studio Studio slice 4 · Marketing lane · the 13th and
 * final tab). Behaviour is unchanged — the whole auto-publish pipeline (compose
 * → schedule → dispatch), take-downs, evergreen library, announce composer, and
 * the manual copy-paste fallback lane move over intact.
 *
 * Two mechanical changes vs the legacy page:
 *   1. It accepts the surface's own searchParams (posted, vendor_posted,
 *      taken_down, settings_saved, pulled, posted_now, retried, body_saved,
 *      announcement_created, evergreen_saved, error) as props from the
 *      /admin/studio shell instead of awaiting them itself. FlashBanner is fed
 *      a reconstructed `search` object so its logic is untouched.
 *   2. The outer max-w-6xl container is dropped (the studio shell provides
 *      layout). Every mutation form's server action still redirects back to
 *      /admin/social-queue?… (which now redirects into ?tab=social-queue) so
 *      their banners surface on the Social queue tab — no action rewrite needed.
 *
 * The cron-free flush ([[project_setnayan_cron_free]]) — viewing the queue fires
 * runSocialFlush() via after() — MOVES with the body so the auto-publish sweep
 * keeps piggybacking on page views from the studio route.
 */
 ...
 */
export async function SocialQueueSurface({
  posted,
  vendor_posted,
  taken_down,
  settings_saved,
...
function FlashBanner({ search }: { search: SearchParams }) {
  if (search.error) {
