/**
 * DiscountCodesSurface — the voucher list body, re-homed byte-identical from
 * app/admin/discount-codes/page.tsx into the tabbed /admin/studio studio
 * (Studio Studio slice 3 · Marketing lane).
 *
 * Only the LIST/index view is absorbed here. The voucher CRUD sub-routes stay
 ...
 *   • Filter strip — All / Active / Disabled / Expired (now ?tab=discount-codes&filter=…)
 *   • Table — code · discount · # services · expires_at · uses · status · actions
 *   • "Create code" CTA in the page header → /admin/discount-codes/new (standalone)
 *
 * Two mechanical changes vs the legacy page:
 *   1. It accepts the surface's own searchParams (filter, created, updated,
 *      disabled, enabled) as props from the /admin/studio shell instead of
 *      awaiting them itself.
 *   2. The outer wrapper is dropped (the studio shell provides layout) and the
 *      list-navigation filter chips point at /admin/studio?tab=discount-codes&…
 *      instead of /admin/discount-codes. The row Edit/Create links + the
 *      disable/enable server actions still return to /admin/discount-codes?… —
 *      the standalone routes and the legacy list redirect keep those flows
 *      landing back on this tab.
 *
 * Cross-references:
