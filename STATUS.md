...
**2026-06-03 — couple detail → live view of their editorial page (0001).** Clicking the bride/groom embeds a phone-framed, same-origin iframe of their public wedding page (`/[slug]`) with Open + Edit links (fallback when no slug yet) — completes the parked couple "album / custom data" item. Bundled a fix for a pre-existing typecheck error on `main` (`e.touches[0]` in the mobile swipe-to-delete card).
