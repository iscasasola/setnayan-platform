-- c6_venue_scene_seated_avatars
-- Created via `pnpm migration:new`. Prefix auto-allocated. KEEP IDEMPOTENT
-- (CREATE OR REPLACE FUNCTION).
--
-- ── WHY (owner 2026-09-06: "build what is not done") ─────────────────────────
-- A guest who made an avatar at /[slug]/avatar walked in as their chibi and was
-- STILL a neutral mannequin in everyone else's room — C5 (20271186016459)
-- shipped only the viewer's OWN config, on the `you` block, and said in its own
-- header: "When the seated rig lands, that block belongs here, on the same
-- venue_photo_visibility gate the photos block directly above uses."
-- The seated rig is the STANDING chibi lowered onto the chair — the rig spec's
-- own sit ("hips to seat, legs dangle (charming, intended)") — drawn by
-- kit/instanced-chibi-crowd.tsx. This is that block.
--
-- ── THE BODY BELOW IS THE LIVE OBJECT, NOT THE LAST MIGRATION ────────────────
-- Copied out of production with pg_get_functiondef on 2026-09-06 and edited in
-- exactly two places (a `v_avatars` declaration + the C6 block and return key),
-- because a CREATE OR REPLACE rebased on an older migration's text silently
-- deletes every change since — which has happened in this schema before.
--
-- Grants are unchanged (anon + authenticated may EXECUTE; that is the whole
-- guest walk). The exposure-freeze guard sees no new capability.
--
