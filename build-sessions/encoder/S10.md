# S10 — a release channel that works, and a download page that tells the truth

**Model:** Sonnet 5 · **Effort:** high · **Wave:** parallel · must land before S11/S12

Measured against `origin/main` @ `bd1c1a39f` on 2026-09-05. Re-fetch before you act.

Part of the S-series encoder plan — `build-sessions/encoder/README.md`. Supersedes E7/E8's unstated prerequisites.

⚠ **S0 MEASURED THE JUSTIFICATION FOR THIS SESSION'S KEEP-AWAKE AND THROTTLING SETTINGS.**
In the real Tauri webview 30 fps held while the page was visible (hardware encode confirmed by
VideoToolbox's own `VCPRateControlSession` log), but the moment the window was occluded WebKit put
the page in Background state, denied GPU access, and VideoToolbox input fell **30 → 16 fps within
three seconds**; after ~8 minutes hidden the page was **suspended entirely**. `encodeQueueSize`
stayed 0 throughout — the loss is upstream of the encoder. So `backgroundThrottling: "disabled"`,
`minimumSystemVersion` and the keep-awake assertion are not hygiene here; they are the difference
between a broadcast and a slideshow. See `S0-FINDING.md` § 4.1.
...
TASK — S10: make it possible to ship a build at all.

MEASURED 2026-09-05 (verify each, paste the command):
  · `.github/workflows/build-desktop.yml` is `workflow_dispatch:` only; its `publish desktop-latest`
    job is `if: github.event_name == 'push'` → UNREACHABLE. `desktop-latest` frozen at 2026-06-19.
  · The repo is PRIVATE → GitHub release asset URLs 404 for the public (`lib/desktop-release.ts`
    says so). `/download` serves ONE committed file `apps/web/public/downloads/Setnayan_0.0.1_aarch64.dmg`
    and has NO Windows link.
  · `tauri.conf.json`: no `minimumSystemVersion` (default 10.13 — a lie on our floor), no
    `backgroundThrottling`, `csp: null`, `TAURI_SIGNING_PRIVATE_KEY: ''` blanked in the workflow.

BUILD:
  1. `publish-latest` runs on `workflow_dispatch` too; uploads `.dmg`, `.msi`, `latest.json` (S12
     will consume it) to a PUBLIC R2 bucket path `desktop/<version>/…` + `desktop/latest/…` (the
     4 R2 buckets exist; pick the public one or add a prefix — say which). `lib/desktop-release.ts`
     resolves from R2. Remove the committed `.dmg` from `public/` once R2 serves it.
  2. `/download`: macOS AND Windows links; a READINESS GATE above them, verbatim: "Works on an
     Apple-silicon Mac on macOS 14 or later with the Safari 26 update, or Windows 10/11 with
     hardware video encoding. Older machines: use OBS (link)." Mirror the sentence into the
     existing `encoderNotice`/readiness surface on the setup page. Detect Intel/older via UA +
     `navigator.platform` best-effort and show the OBS path first for them.
  3. `tauri.conf.json`: `bundle.macOS.minimumSystemVersion: "14.0"`; window
     `backgroundThrottling: "disabled"`; add `tauri-plugin-prevent-sleep` (or an IOPM/
     `SetThreadExecutionState` command — say which) held between `encoder_start` and `stop`.
  4. Windows: a job-summary line saying "UNSIGNED — SmartScreen will warn" until S11; the rehearsal
     script (S13) teaches "More info → Run anyway".

GUARDS: a test that `desktop-release.ts` URLs are absolute R2 URLs (sabotage → red); the readiness
sentence present on `/download` and setup (sabotage → red).

EVIDENCE: a fresh browser downloads BOTH artefacts from `/download`; `gh run` shows publish-latest
ran under dispatch.
