# S1 — the program surface becomes an OffscreenCanvas in a worker

**Model:** Sonnet 5 · **Effort:** high · **Wave:** after S0 starts (does not need its result)

Measured against `origin/main` @ `bd1c1a39f` on 2026-09-05. Re-fetch before you act.

Part of the S-series encoder plan — `build-sessions/encoder/README.md`. Supersedes E1.
...
TASK — S1: composite the program output on an OffscreenCanvas, in a worker, in-process.

RULE 0 HITS (extend, do not rebuild):
  · `apps/web/lib/panood-program-bridge.ts` — `installProgramBridge` / `subscribe`, publishes
    `ProgramFrame{ source, label, live, stream, secondaryStream, splitRatio, requestedSource, … }`
    on the SAME window as the controller. `EMPTY_FRAME` exists and is tested (18 tests).
  · `app/panood/control/[eventId]/_components/program-surface.tsx` — the DOM pop-out: `<video>`
    + `NoSignalCard` + `WithheldCard` + `PinnedChannelNotice` + `ObsOrderingNotice`. Export its
    STRINGS so the canvas draws the same words. Do not touch its rendering.
  · `apps/web/lib/reel-render.ts` — canvas + WebCodecs precedent (codec probe list,
    `latencyMode`). Copy the shape, not the file.
  · `clampSplitRatio` — reuse; see rule 21.

BUILD `apps/web/lib/encoder/` (new dir, nested — no allowlist issue):
  · `program-canvas.worker.ts`: owns a 1280×720 OffscreenCanvas; receives the on-air video
    track via `MediaStreamTrackProcessor` (transfer the track's `ReadableStream`; Safari: worker
    only, video only). Draw loop driven by a worker timer at 33.3 ms for now — MARK THE SEAM with
    `// S3 replaces this tick with the AudioContext-derived clock` — S3 owns the master clock.
  · Repeat-last-frame on stall: if no new VideoFrame by the tick, re-draw the last composite.
    Never gap, never freeze the tick.
  · `draw(frame: ProgramFrame)`: `stream` null → `NoSignalCard` text; `requestedSource !==
    source` → `WithheldCard` text; `EMPTY_FRAME` → "Nothing on program yet" placeholder, NEVER a
    paywall or watermark. `secondaryStream` non-null → split at `clampSplitRatio(splitRatio)`;
    otherwise full-frame. Overlays: NOT here — S2.
  · `program-canvas.ts` (main thread): subscribes to the bridge, forwards track changes and
    `ProgramFrame` to the worker, exposes `start()/stop()/onFrameCount`. Gated on
    `window.__TAURI__` at the call site later (S5); the module itself is plain browser code.

GUARDS (mutation-test each; print occurrence counts before → after):
  · null `stream` draws the placeholder, not the previous camera's last frame.
  · `requestedSource ≠ source` draws Withheld.
  · `EMPTY_FRAME` contains no overlay/watermark draw call.
  · split honoured only when `secondaryStream` non-null (synthetic frame test).
Frame-hash tests over synthetic `VideoFrame`s (node: use `@napi-rs/canvas` or skip pixel tests
and assert the draw-call log — choose one, say which).

EVIDENCE: a 10-minute run in the real app with the window fully occluded shows a continuous
frame counter (no gaps > 2 ticks). Record the counter output.
