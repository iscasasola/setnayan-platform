...
- **`orders.pax_snapshot`** (migration `20271008000839`) records the pax an order was priced at, written once at insert and never recomputed. Follows the `orders.setnayan_fee_bps` precedent on the same table. It is an **audit/lineage record, not an enforcement boundary** — see the correction below.

**⚠ Correction made before merge — the column REVOKE was wrong twice, and both were verified, not assumed.** The first revision of this migration tried `REVOKE UPDATE (pax_snapshot) ON public.orders FROM authenticated`:

1. **It is a no-op.** Postgres cannot subtract a column from a table-level grant, and `authenticated` holds table-level INSERT/UPDATE on `public.orders` through Supabase's platform default privileges. After the REVOKE, `has_column_privilege('authenticated', …, 'pax_snapshot', 'UPDATE')` is *still* true. This is the same trap documented at the top of `20271006100000`, and exactly why `20271005100000` had to revoke the **table-level** privilege first and grant an explicit column list back.
2. **If it had worked it would have broken checkout.** The only writer, `app/dashboard/[eventId]/checkout/actions.ts:602` (`submitOrderAction`), inserts through `createClient()` — the session-bound **authenticated** client, not `createAdminClient()`. Denying `authenticated` INSERT on the column would reject every pax-priced order at the till.

Because of (1) the migration's own post-condition then failed in **both** environments — `service_role-lost-insert` under the replay harness (which reds the entire DB-replay suite, see Tests) and `authenticated-can-still-update, anon-can-still-update` against a prod-like grant state, which would have **aborted `supabase db push`**. The REVOKE and the privilege assertions are removed; the post-condition now asserts catalog shape only, which is the part this file actually establishes. The behavioural money fix (`resolveLivePax()` above) is untouched and is what closes the exploit.
...
`pnpm test:unit` — **3899 pass, 0 fail**; `pnpm test:db` — **261 pass, 0 fail**; `tsc --noEmit` clean. (The "3 pre-existing local failures" reported on the first revision were only an uninstalled `node_modules`; with deps installed the suite is fully green.) New coverage:
...
No DB test was written here; the migration carries its own post-condition block, which cannot pass vacuously the way an owner-connected RLS test can. It now asserts **catalog shape only** (the column exists and is `integer`; the CHECK exists and is a real check constraint). It deliberately makes **no `has_*_privilege()` assertion**: role grants are environment state this file does not establish — Supabase's default privileges grant the API roles `ALL` on public tables, while the replay harness (`apps/web/tests/db/replay-migrations.ts:283`) mirrors that GRANT only *after* the whole corpus has replayed. A privilege assertion therefore reads a different world in each environment and proves nothing about this migration. That mismatch is precisely what took CI down: the migration RAISEd mid-replay, `createReplayedDb` aborted, and **every** DB-replay test failed with it — including the five Papic camera-point metering subtests, which have nothing to do with pax. Both directions were reproduced against a prod-like and a replay-like grant state before and after the fix.
...
- **🔶 OWNER DECISION — making `orders.pax_snapshot` genuinely un-forgeable.** As shipped it is an audit record a determined payer could still write, because the order INSERT runs as the `authenticated` client. Closing it means either (a) restructuring the grants on `public.orders` the way `20271005100000` did for `public.events` (table-level REVOKE + explicit column re-GRANT), or (b) moving the order INSERT to the service-role client. Both land squarely in the checkout/orders work currently in flight, so neither was taken here. Note the row already has a **strictly larger hole in the same INSERT** — `requested_total_php` is client-supplied (next bullet) — so hardening one audit column while the amount beside it is still client-typed buys nothing on its own. These two should be fixed together, as one deliberate pass over order-creation trust.
