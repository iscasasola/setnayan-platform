 ...
 * m-card surfaces, gold (#A9834B) accents, obsidian (#1B1A17) ink.
 */
...
> = {
  ...
  OTHER: {
    label: 'Other',
    badge: 'bg-[#A9834B]/12 text-[#8A6B39] border-[#A9834B]/30',
    Icon: Activity,
  },
};
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <div className="m-card flex items-center justify-between px-4 py-3">
          <div>
            <p className="m-label-mono text-[10px] uppercase tracking-[0.18em] text-[#8A6B39]">
              Live stream
            </p>
            <p className="mt-1 text-sm font-medium text-[#1B1A17]">
              {live ? 'Connected' : 'Connecting…'}
            </p>
          </div>
          <span
            className={`inline-block h-2.5 w-2.5 rounded-full ${
              live ? 'animate-pulse bg-success-500' : 'bg-[#A9834B]/40'
            }`}
            aria-hidden="true"
          />
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {tab === 'active' && active.length > 0 ? (
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-full border border-[#A9834B] bg-[#A9834B] px-4 py-1.5 text-xs font-medium text-white transition hover:bg-[#8A6B39] disabled:opacity-50"
          >
      ...
      {tab === 'active' ? (
        ...
        >
          <span className="flex items-center gap-1.5 text-xs uppercase tracking-[0.18em] text-[#8A6B39]">
          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={`inline-flex items-center rounded-full border px-3 py-1 text-xs transition ${
                filter === f.key
                  ? 'border-[#A9834B] bg-[#A9834B] text-white'
                  : 'border-[#A9834B]/30 bg-white text-[#1B1A17] hover:bg-[#FBFBFA]'
              }`}
            >
              {f.label}
            </button>
      ...
      {tab === 'active' && active.length >= rowLimit ? (
        <p className="text-center text-[11px] uppercase tracking-[0.16em] text-[#8A6B39]">
    ...
    <li className="m-card group flex items-start gap-3 px-4 py-3">
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block truncate text-sm font-medium text-[#1B1A17]">
        ...
        <ChevronRight
          className="mt-1 h-4 w-4 shrink-0 text-[#A9834B]/50 transition group-hover:text-[#A9834B]"
          aria-hidden="true"
        />
      </button>
      ...
      {isActive ? (
        <div className="flex shrink-0 items-center gap-1.5">
          <button
            ...
            className="inline-flex items-center gap-1 rounded-full border border-[#A9834B]/30 bg-white px-2.5 py-1 text-xs text-[#5B5B5B] transition hover:bg-[#FBFBFA] disabled:opacity-50"
          >
    ...
    <div className="m-card flex flex-col items-center justify-center gap-3 px-6 py-14 text-center">
      {tab === 'active' ? (
        <>
          ...
          <p className="text-base font-medium text-[#1B1A17]">All clear</p>
        ...
        <>
          <Inbox className="h-8 w-8 text-[#A9834B]" aria-hidden="true" />
          <p className="max-w-md text-sm leading-relaxed text-[#5B5B5B]">
...
function InspectModal({ row, onClose }: { row: FaultLogRow; onClose: () => void }) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-center bg-[#1B1A17]/40 p-0 backdrop-blur-sm sm:items-center sm:p-6"
      onClick={onClose}
      role="presentation"
    >
      <div
      ...
      >
        <header className="flex items-start justify-between gap-3 border-b border-[#A9834B]/15 px-5 py-4">
          <div className="min-w-0">
            ...
            <h2 className="mt-2 truncate text-lg font-semibold text-[#1B1A17]">
              {row.element_name || 'Unnamed element'}
            </h2>
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 rounded-full border border-[#A9834B]/30 p-1.5 text-[#5B5B5B] transition hover:bg-[#FBFBFA]"
            aria-label="Close"
          >
        ...
        <div className="space-y-4 overflow-y-auto px-5 py-4">
          <Field label="File path">
            <code className="m-mono block break-all rounded-lg bg-[#FBFBFA] px-3 py-2 text-xs text-[#1B1A17]">
              {row.file_path || '—'}
            </code>
          </Field>

          <Field label="Error">
            <pre className="m-mono max-h-48 overflow-auto whitespace-pre-wrap break-all rounded-lg bg-[#1B1A17] px-3 py-2 text-xs leading-relaxed text-[#FBFBFA]">
              {row.error_message || '—'}
            </pre>
          </Field>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="m-label-mono mb-1 text-[10px] uppercase tracking-[0.18em] text-[#8A6B39]">
        {label}
      </p>
      {children}
    </div>
...
function JsonTree({ data, depth = 0 }: { data: unknown; depth?: number }) {
  ...
  if (Array.isArray(data)) {
    ...
    return (
      <ul className="border-l border-[#A9834B]/20 pl-3">
        {data.map((item, i) => (
          <li key={i} className="text-xs">
            <span className="text-[#8A6B39]">{i}: </span>
  ...
  if (t === 'object') {
    ...
    return (
      <ul className={depth === 0 ? '' : 'border-l border-[#A9834B]/20 pl-3'}>
        {entries.map(([k, v]) => (
          <li key={k} className="text-xs leading-relaxed">
            <span className="font-medium text-[#1B1A17]">{k}</span>
...
function StatCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="m-card px-4 py-3">
      <p className="m-label-mono text-[10px] uppercase tracking-[0.18em] text-[#8A6B39]">{label}</p>
      <p className="mt-1 text-2xl font-semibold text-[#1B1A17]">{value}</p>
    </div>
  );
}
  ...
  return (
    <span
      className={`inline-flex min-w-[1.25rem] items-center justify-center rounded-full px-1.5 text-[10px] ${
        active ? 'bg-white/20 text-white' : 'bg-[#A9834B]/15 text-[#8A6B39]'
      }`}
    >
      {n}
    </span>
