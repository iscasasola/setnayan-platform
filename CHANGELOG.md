...
**Verification:** `tsc --noEmit` exit 0 · `next lint` clean (zero warnings in the changed file). Production-build CI validates the RSC/client boundary (drawer props are all serializable — strings + a plain settings object, same shape the 7 add-on pages already pass).

**SPEC IMPACT:** First-party **Setnayan services** are now genuinely **add-and-pay inline** via the canonical apply-then-pay flow (0034) with admin acceptance at `/admin/payments` — completing the "in-app services = vendor listings · add-and-pay via 0034" model. This is the **interim** mechanism until the automated payment system (2027-01-01). Land directly in the corpus (`DECISION_LOG.md` + `0006`/`0021`/`0034`): note (a) the reuse of `InlineCheckoutDrawer` + `/admin/payments` for first-party service picks, (b) the deliberate first-party disclosure (NOT the vendor non-custody banner), (c) the `setnayan_service__{category}` keying convention and its one-per-category status-correlation limitation.
