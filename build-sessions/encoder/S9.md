# S9 — the couple can see the encoder — health states and adaptive bitrate

**Model:** Sonnet 5 · **Effort:** high · **Wave:** after S5 and S6

Measured against `origin/main` @ `bd1c1a39f` on 2026-09-05. Re-fetch before you act.

Part of the S-series encoder plan — `build-sessions/encoder/README.md`. Supersedes E4's 'extend the strip' made concrete; ABR was never in the E-series.
...
TASK — S9: extend LS4's surface with the encoder's state, and step the bitrate before frames pile up.

RULE 0 (rule 24): `lib/live-studio-ingest-health.ts` — pure `decideIngestHealth({ streamStatus,
healthStatus, live, lastOkAt })` → `waiting_for_encoder|receiving|degraded|no_data`;
`POLL_INTERVAL_MS=150_000`; `IngestHealthStrip` polls, mounted ONLY when
`liveAir.source === 'broadcast'`; PR #5122, 15 tests, 5 mutation-tested assertions. EXTEND IT.

HEALTH:
  · `IngestHealthInput.encoder?: { rtmp:'idle'|'connecting'|'publishing'|'reconnecting'|'down';
    reconnectingForMs:number; droppedFrames:number; bitrateRung:0|1|2; recording:boolean } | null`.
  · `IngestHealthState` += `'reconnecting' | 'encoder_down'`. The strip's
    `Record<IngestHealthState,…>` skins fail typecheck until drawn — draw them.
  · Rust → JS over S5's `Channel<HealthEvent>` → the SAME `decideIngestHealth` call. One decider.
  · PRECEDENCE: YouTube `noData`/stale ALWAYS beats local `publishing` (local can only make the
    strip louder/faster, never greener); local `reconnecting`/`down` pre-empts `receiving` within
    1 s.
  · MOUNT RULE: today only for `source === 'broadcast'`; add "or the desktop encoder is running",
    so an own-channel broadcast shows local RTMP state and `no_data("cannot tell — your own
    channel")` for the YouTube half instead of nothing.

ADAPTIVE BITRATE:
  · Rust samples unsent socket bytes every 500 ms (`SO_NWRITE` on macOS via `getsockopt`;
    Windows: bytes-written minus completed-write bytes) → `Channel<SendBufferOccupancy>`.
  · JS ladder 2.5 → 1.8 → 1.2 Mbps (720p30 → 720p30 → 540p30) via `VideoEncoder.configure()`
    in place: DOWN after 2 s above 1.5 s of buffered send; UP after 30 s clean. Rung is a health
    sub-state ("streaming at reduced quality").
  · Default 720p30 @ 2.5 Mbps for PH mobile upload ~10 Mb/s.

GUARDS (mutation-test, counts): local `publishing` overriding YouTube `noData` → red; ladder
without an upper bound / hysteresis → red; strip mounted for own-channel without encoder → red;
existing 15 tests still green (paste count).

EVIDENCE: throttle the uplink to 1.5 Mbps (`tc`/Network Link Conditioner): rung steps down within
3 s and the strip says so; restore: steps up after 30 s. Paste the log.
