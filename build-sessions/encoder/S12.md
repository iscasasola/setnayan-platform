# S12 — the encoder can be fixed without asking couples to reinstall

**Model:** Sonnet 5 · **Effort:** high · **Wave:** after S11

Measured against `origin/main` @ `bd1c1a39f` on 2026-09-05. Re-fetch before you act.

Part of the S-series encoder plan — `build-sessions/encoder/README.md`. Supersedes E8.
...
TASK — S12: an auto-updater. Owner-locked as part of the estimate (DECISION_LOG 2026-09-03 ②).

PREMISE (true): the shell is a meta-refresh to setnayan.com — WEB changes reach users instantly;
only the NATIVE half (Rust encoder) is version-locked. PREMISE E8 MISSED: the repo is private, so
the update endpoint is S10's R2 `latest.json`, never a GitHub release URL.

BUILD:
  · `tauri-plugin-updater` 2.x; `bundle.createUpdaterArtifacts: true`;
    `plugins.updater.{ pubkey, endpoints:[R2 latest.json] }`; S11's keypair signs artefacts.
  · Check from RUST at launch (not from the web page — the web page is the same for everyone),
    and DEFER while `encoder_state != idle` (S5 keeps that state). Windows exits the app to
    install — "never mid-broadcast" is a platform requirement, not a nicety. Re-check on
    `encoder_stop`.
  · A small native notice ("Update ready — installs when you're not streaming") and nothing
    modal during a broadcast.
  · `publish-latest` (S10) writes `latest.json` with signatures for both platforms.

GUARDS (mutation-test, counts): update attempted while publishing → red; unsigned manifest
accepted → red; endpoint not R2 → red.

EVIDENCE: 0.0.1 → 0.0.2 on BOTH OSes with a stream running: no restart; after stop: installs.
Paste the updater log.
