# E9 — a rehearsal that proves it, end to end

**Model:** Sonnet 5 · **Effort:** medium · **Wave:** last
...
TASK — E9: an acceptance run on both operating systems. The deliverable is a REPORT, not a fix.

🛑 IF SOMETHING FAILS, WRITE IT DOWN AND CARRY ON. Fixing mid-run turns an acceptance test into
a debugging session and you lose the honest picture of what a couple would have experienced.

RUN THE WHOLE THING, on macOS and on Windows:
  1. Install from the real artefact, as a first-time user. Note every dialog.
  2. Two or more phones join by QR. Confirm they reach the controller.
  3. Cut between them. Split-screen at a non-half ratio.
  4. Overlays: monogram and lower third correct on the ENCODED output, not just the console.
     ⚠ Check the FREE-tier path too — an un-entitled event must still carry POWERED BY
     SETNAYAN in the encoded frames. This is the revenue property E2 exists to protect and the
     only place it is checked against real bytes.
  5. Go live. Confirm the picture reaches YouTube and the watch URL plays.
  6. Let it run long enough to matter — an hour minimum. Note thermals and drift between audio
     and picture.
  7. KILL THE NETWORK MID-STREAM. Confirm: it reconnects, the ingest-health strip goes honest
     rather than silent, and it never claims "receiving" while nothing arrives.
  8. Stop. Confirm the recording exists and the replay reaches the couple's story page.

REPORT: what a couple would actually experience, step by step, with the failures listed
plainly. An acceptance run that reports only successes has not been run properly.
