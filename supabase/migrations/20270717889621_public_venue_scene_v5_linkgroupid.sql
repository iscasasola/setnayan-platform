-- public_venue_scene v5 — expose link_group_id per table so the guest 3D walk
-- can render LINKED serpentine chairs at the same even spacing the 2D editor +
-- 3D lab + PDF pack already use (audit gap: the guest walk hardcoded
-- linkGroupId:null because v4 never returned it).
--
-- IDENTICAL to the live v4 definition (captured via pg_get_functiondef) EXCEPT
-- the one added field `'linkGroupId', t.link_group_id` in the tables block.
-- CREATE OR REPLACE — idempotent, signature unchanged (text, text), so nothing
-- else needs touching.
