-- public_venue_scene v8 — carry the booked vendor's TIER + marketplace SLUG +
-- BOOKABLE flag to the public guest walk, so a published venue page renders the
-- same booth the couple and the vendor already see.
--
-- WHY (live bug): v7's vendor block selected only name/category/logoUrl. The
-- renderer gates logo branding on boothCanBrand(tier) — tier was absent, so it
-- read undefined and EVERY public booth rendered generic. Pro/Enterprise logo
-- branding is built, gated and sold, yet invisible in the one place guests see
-- it. The booth card's "/v/[slug]" CTA was dark for the same reason.
--
-- Mirrors the authenticated path EXACTLY (apps/web/lib/seating.ts:1853-1856):
...
-- where lib/vendor-visibility.ts defines
--   isPubliclyVisible := visibility IN ('coming_soon','verified')
--   isBookable        := visibility  =  'verified'
--   parseVisibility(NULL) := 'coming_soon'   <- the COALESCE below
-- A manual / off-platform booth (no marketplace_vendor_id, so vp IS NULL)
-- yields tier NULL, slug NULL, bookable false — unchanged behaviour.
--
-- STILL PUBLIC BUSINESS IDENTITY ONLY — no PII added. tier/slug/bookable are
-- the same three fields already public on /v/[slug].
--
-- Idempotent CREATE OR REPLACE. IDENTICAL to v7 EXCEPT the three added keys on
-- the booth vendor jsonb_build_object. Signature unchanged (text, text); STABLE
-- SECURITY DEFINER + search_path + every v6/v7 join preserved.
--
-- NOT in this migration: booth `cardItems`. That resolver is a ~90-line
-- multi-table composition (category-match-beats-first-active over
-- vendor_services -> vendor_service_inclusions, host_inclusions fallback,
-- fail-soft) in lib/vendor-services.ts fetchBoothCardItems. Porting it to
-- plpgsql is its own PR; guests keep the thinner card until then.
