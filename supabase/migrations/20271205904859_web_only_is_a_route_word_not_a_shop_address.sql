-- web_only_is_a_route_word_not_a_shop_address
--
-- The App Store shell (Capacitor iOS/Android) now lands on a real top-level
-- page, /web-only, whenever it reaches a paid digital feature — App Review
-- 2026-06-30, Guideline 3.1.1 via 3.1.3(b); see lib/store-shell.ts. A
-- top-level route means `web-only` must stop being a word the shop-address
-- mint can hand out.
--
-- WHY THIS IS URGENT RATHER THAN TIDY: a shop address is IMMUTABLE once minted.
-- A business named "Web Only" would hold setnayan.com/web-only forever and the
-- page the store shell is sent to could never live there — the same trap that
-- nearly cost us /creators and /open-shop on 2026-08-11, and the reason `pay`,
-- `pakanta` and `mood-board` were each reserved the day their page appeared.
--
-- The app-side list (`lib/reserved-slugs.ts`, generated from the route folders)
-- already carries it — the folder appeared, so the generator picked it up.
...
-- is the check that sent us here, and it was the ONLY thing failing on #5186.
--
...
-- (2026-09-05):
--
--   select business_slug_is_reserved('web-only'),
--          (select count(*) from vendor_profiles where lower(business_slug) = 'web-only'),
--          (select count(*) from events          where lower(slug)          = 'web-only'),
--          (select count(*) from users           where lower(slug)          = 'web-only');
--   → false, 0, 0, 0
--
...
-- this text, and the ONLY difference is the 'web-only' entry added below.
...
AS $function$
    SELECT lower(coalesce(p_slug, '')) = ANY (ARRAY[
      ...
      -- introduce.
      'mood-board',
      -- ⬇ ADDED 2026-09-05. Where the App Store shell lands when it reaches a
      -- paid digital feature it may not show (App Review 3.1.1 / 3.1.3(b),
      -- lib/store-shell.ts). Reserved the day the route folder appeared.
      'web-only',
