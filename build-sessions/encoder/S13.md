# S13 — a rehearsal that proves it, end to end — the deliverable is a report

**Model:** Sonnet 5 · **Effort:** medium · **Wave:** last · from the REAL installers

Measured against `origin/main` @ `bd1c1a39f` on 2026-09-05. Re-fetch before you act.

Part of the S-series encoder plan — `build-sessions/encoder/README.md`. Supersedes E9 (split step removed; recording, minimise, lid, guest phone added).
...
TASK — S13: run the day the way a couple would, on both operating systems, and write down what happened.

🛑 The deliverable is `build-sessions/encoder/S13-REPORT.md`. Fixes found here are NEW sessions,
not this one — unless a one-line fix unblocks the run, in which case say so.

SETUP: install from `/download` on a clean Apple-silicon Mac (macOS 14 or 15 + Safari 26) and a
clean Windows 11 laptop, plus ONE macOS 13 or Intel machine EXPECTED TO FAIL at the readiness
gate (not at go-live — that is the test). Two phones as cameras, one with the mic refused. A
guest's phone on the story page for the whole run. Own-channel route on one OS, hosted-channel on
the other. Record every dialog the OS shows, verbatim, including SmartScreen "Run anyway".

THE RUN (60+ minutes per OS):
  1. Go live from the app. YouTube shows "Excellent"; the strip shows publishing.
  2. Cut between cameras every few minutes; cut to the mic-less phone for 5 min (silence, not
     dead air).
  3. Minimise the window for 10 minutes. Frame counter continuous? (WebView2 is the one to watch.)
  4. Kill the network three times (10 s, 45 s, 3 min). Note which reconnects kept the broadcast
     and which needed a new one; the guest phone's link must follow (W1). Record YouTube's actual
     grace window — the first measurement we will have.
  5. Throttle the uplink to 1.5 Mbps for 2 min; the rung steps down, the strip says so.
  6. Lid rule: state it in the script ("lid open, power in, do not close, do not minimise") and
     then CLOSE the lid once, on the Mac, without an external display. Record what happened.
  7. Stop. The `.flv` path is shown; it plays in VLC end to end. The YouTube archive reaches the
     story page.
  8. Free-tier branding: on the free tier, `ffmpeg -i <archive> -frames:v 1 -ss <t>` at three
     timestamps shows the forced lower third in the RECORDED bytes.
  9. Update: bump to a test version on R2; the app does not restart mid-stream; after stop it does.

THE REPORT: per step, per OS: PASS / FAIL / PARTIAL with the evidence, plus "what the couple
would have experienced" in one plain sentence each. A FAIL is a finding, not a defeat — list it
plainly. End with the rehearsal script for couples, in their words, built from what you saw.

HAND BACK in rule 16 format. PR = the report. Auto-merge.
