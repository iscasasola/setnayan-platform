# L3 — the film on the product page is the shipped interface

**Model:** Sonnet 5 · **Effort:** medium · **Wave:** after L1 · **GATED: do not start until a camera's picture reaches the controller**

Measured against `origin/main` @ `7d6206679` on 2026-09-01. Re-fetch before you act.
...
TASK — L3: give /panood the product film Papic already has.

⛔ GATE: this session cannot complete until L1 lands. A film of a product that cannot show a picture
would lock in the broken version. If L1 has not merged, STOP and say so — that is a complete session.

WHAT ALREADY EXISTS, and is the pattern to mirror:
  · `app/(shell)/papic/_papic-film.tsx` — a 12-second silent loop, and its docblock states the rule
    this session exists to honour: 'Not a mock-up: a recording of the same scenes the live demo
    renders, so it can never drift from the product.'
  · It is a CLIENT component on purpose: a bare autoPlay video that a browser refuses becomes a dead
    still with no controls, so a refused play falls back to handing the viewer the browser's own
    controls. Reproduce that fallback; do not simplify it away.
  · `/panood` already has a live demo doorway — studioApps 'panood' → demo 'Try the control room
    with two phones'. The film is what is missing, not the demo.

OWNER CONSTRAINT, verbatim 2026-09-01: 'the whole interface must be the same as what we offer.'

SCOPE:
  1. Add the film component to `app/(shell)/panood/page.tsx`, mirroring the Papic shape including
     the autoplay-refused fallback.
  2. AUDIT THE PAGE'S CLAIMS AGAINST WHAT THE FILM CAN SHOW. Every promise above the fold must be
     visible in the recording. Anything the shipped product cannot demonstrate comes OFF the page or
     is reworded — do not leave a claim standing because it is aspirational.
  3. Report which claims you removed or changed and why. That list is the deliverable, not just the
     component.

⚠ THE RECORDING ITSELF IS THE OWNER'S TO MAKE — you are not asked to fabricate footage. If no asset
exists yet, build the component against the real asset path, leave it rendering nothing until the
file lands (the Papic component's own empty-state discipline), and say so in the handback.

⚠ TWO CUTS COME FROM ONE SESSION, and they are NOT the same file. The marketing loop is ~12s, silent,
and must NEVER show the OAuth consent screen. Google's verification video is 2–5 minutes, narrated,
and MUST show the consent screen and the scope being used. This session owns only the first.
