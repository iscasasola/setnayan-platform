...
| **No limit on customers per date** | **+₱2,500** ✅ (owner 2026-08-29) | Removes the per-tier ceiling on customers a shop may be **chasing** for ONE date (accepted-but-not-yet-locked). Custom's base is 10, the same as Enterprise. Asked what going past 10 should cost, the owner answered ***"2500 for no limit."*** ⛔ **The BOOKED-OUT WAITLIST is NOT lifted** — a different list (couples queued on a date already sold), still 5, and widening it would also mean widening a CHECK constraint. He was asked about the 10 and answered about the 10. Enforced in the database (`vendor_pipeline_is_unlimited` → `enforce_vendor_whitelist_per_date`), not only in the caps overlay, and the on-screen "you're chasing N of 10" line goes silent for a shop that holds it |
