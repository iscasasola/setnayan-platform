...
const DELIBERATE_EXCLUSIONS: Record<string, string> = {
  ...
  // TABLES DROPPED 2026-09-18 (migration 20271234122426, S39) — neither ever
  // had a writer; 0 rows in prod. Listed only because readSchema() unions every
  // historical CREATE TABLE and never reads DROP TABLE (same as
  // calendar_feed_tokens). vendor_meetings is ALSO still pinned by name in T7
  // as a PARSER canary — that assertion is about the parser, not the table.
  vendor_self_comp_caps:
    'TABLE DROPPED 2026-09-18 (S39) — per-store comp-cap override, never written. Nothing to export.',
  vendor_meetings:
    'TABLE DROPPED 2026-09-18 (S39) — ad-hoc meetings table, never written; appointments replaced it. Nothing to export.',
