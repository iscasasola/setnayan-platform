...
**Current sprint:** Iteration `0023_admin_console` MVP shipped — 3 of 7 surfaces. New `/admin` route tree, gated to users where `is_internal OR is_team_member OR account_type='admin'`; non-admins get a 404 (not a redirect — the admin route shouldn't even appear to exist for non-staff). Overview page shows live counts (all users · couples · vendor users · events · vendor profiles · chat threads · 🟣 internal · 🟢 team pool) via service-role count queries. Users surface: server-rendered table with search (email/name/public_id) + 5-way filter (all / customer / vendor / internal / team pool); "Add to pool" / "Remove from pool" buttons toggle `is_team_member` for non-internal accounts via a `requireAdmin`-guarded server action that uses the service-role client. Events surface: 200-row table with date / venue / slug / live guest count (joined from `guests` where `deleted_at IS NULL`) + archived toggle. Vendors surface: grid of vendor profile cards with logo/initials avatar, published vs. draft pill, services preview, public_id. Admin-console link surfaces on `/dashboard/profile` for internal/team-pool users only. Two-admin approval queue, audit log, system health, settings remain deferred.
