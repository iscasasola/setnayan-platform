# W1 — what guests see — the watch link follows the broadcast

**Model:** Sonnet 5 · **Effort:** high · **Wave:** parallel · path-independent

Measured against `origin/main` @ `bd1c1a39f` on 2026-09-05. Re-fetch before you act.

Part of the S-series encoder plan — `build-sessions/encoder/README.md`. Supersedes nothing — the E-series never covered guests.
...
TASK — W1: guests never hold a dead link.

RULE 0: the story page's `watch-live-block.tsx` server-renders `watchLive.watchUrl` as a STATIC
`<a href>`; there is no client re-resolution. A reconnect that had to create a new broadcast
(`bindYoutubeBroadcast` on the same stream — S7 emits `BroadcastEnded`) is a NEW URL, and 200
guests are on the old one. Own-channel by-hand hosts paste a watch link (`live-studio-manual-air.ts`)
— the same block, the same problem if they restart.

BUILD:
  · `GET /api/live/[slug]/watch` → `{ watchUrl, state:'live'|'reconnecting'|'ended'|'not_yet' }`,
    public, cached 15 s, reads what the story page already reads (find it; do not add a query).
  · In `watch-live-block.tsx`: while `state==='live'|'reconnecting'`, poll every 30 s and swap the
    href in place; between parts show "The stream is reconnecting — this link will update on its
    own." Part one's archive stays reachable from the recordings card (verify that exists).
  · Server side: when S7's `BroadcastEnded` arrives (via the existing ingest-health route or a new
    host-gated `POST /api/live-studio/encoder/broadcast-ended`), the existing provisioning creates
    the next broadcast on the same stream and updates `panood_watch_url`. Reuse
    `bindYoutubeBroadcast`; do not write a second binder.

GUARD (mutation-test): a changed `panood_watch_url` reaches the page without reload (sabotage
the poll → red); "ended" never shows "reconnecting" (→ red).

EVIDENCE: swap the URL in the DB during a live page view on a phone; the href changes within 30 s.
