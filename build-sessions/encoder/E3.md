# E3 — H.264 video and AAC audio, from WebCodecs

**Model:** Sonnet 5 · **Effort:** high · **Wave:** after E1
...
TASK — E3: encode the canvas to H.264, and the programme audio to AAC.

VIDEO is the proven half — the repo already encodes H.264 (`lib/boomerang-encoder.ts` sets
`latencyMode: 'quality'`; you want the realtime posture instead). AUDIO IS NEW and is the
half to budget for.

⚠ AUDIO IS WHY THIS DESIGN EXISTS AT ALL. It comes off the `MediaStream` as a TRACK, which is
precisely what removes the manual Desktop Audio Capture step OBS needs — the step that can
silently lose the vows to an OS routing setting. Take the audio from the transport, never
from a system device.

YOUTUBE'S INGEST REQUIREMENTS — check them against your config and write the arithmetic in a
comment: H.264 video, AAC audio, 2-second keyframe interval, CBR. A stream that negotiates but
drifts off keyframe cadence degrades in a way viewers see and the health strip will not catch.

  · `VideoEncoder` config from the E0 finding's proven codec string, not a guess.
  · `AudioEncoder`, `mp4a.40.2`, 48 kHz stereo.
  · Timestamps must be a single monotonic clock shared by both tracks. Two clocks is how
    audio drifts from picture over a six-hour event — inaudible at minute one, obvious by the
    reception.

GUARD: the config asserts the four YouTube requirements; a keyframe-interval change turns it
red. MUTATION-TEST. Full suites. COMMIT BEFORE A LONG VERIFY.
