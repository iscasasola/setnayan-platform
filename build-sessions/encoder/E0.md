# E0 — the spike — does any of this work inside Tauri?

**Model:** Opus 5 · **Effort:** high · **Wave:** FIRST · BLOCKING
...
TASK — E0: answer two questions that can reshape the whole encoder plan, then stop.

🛑 THIS SESSION WRITES A FINDING, NOT A FEATURE. Every session after this one assumes the
answers below. Do not start building the encoder here.

QUESTION 1 — DOES THE TAURI WEBVIEW EXPOSE WEBCODECS?
`VideoEncoder` AND `AudioEncoder`, on macOS (WKWebView) and Windows (WebView2). WebCodecs is
confirmed shipped in Safari 26 — video since 16.4, audio added in 26 — but whether WKWebView
exposes it is UNDOCUMENTED EITHER WAY. Do not infer it from Safari; run it.

  · Build and launch the desktop app, and evaluate in the real webview:
      typeof VideoEncoder, typeof AudioEncoder
      await VideoEncoder.isConfigSupported({ codec: 'avc1.42001f', width: 1280, height: 720, bitrate: 2_500_000 })
      await AudioEncoder.isConfigSupported({ codec: 'mp4a.40.2', sampleRate: 48000, numberOfChannels: 2 })
  · Record the OS version and webview version alongside the answer — "works on my Mac" is not
    the finding; "works on WKWebView <version>" is.

QUESTION 2 — DOES THE DESKTOP BUILD STILL COMPILE?
`.github/workflows/build-desktop.yml` is `workflow_dispatch:` only and HAS NOT RUN SINCE
2026-06-25 (owner deferred it 2026-06-20 to save Actions minutes). `main` has moved hundreds
of commits since. Trigger it — Actions → build-desktop → Run workflow — and report whether BOTH
matrix legs (macOS aarch64, Windows x86_64-msvc) still produce a bundle.

WHAT TO DO WITH EACH ANSWER:
  · Both yes → the plan stands. Say so and stop.
  · WebCodecs missing → the composite/encode half cannot live in the webview. Report the
    options honestly (native encode via VideoToolbox/Media Foundation, or an ffmpeg sidecar
    doing encode as well as mux) with a re-estimate. DO NOT pick one.
  · Build broken → repair is now part of the estimate. Report what broke and what it costs.

⚠ AN UNSIGNED WINDOWS `.msi` TRIGGERS SMARTSCREEN. The workflow header says "Windows is still
unsigned (Phase 2)". Note it — it is E7's problem, not yours, but the finding belongs here.

HAND BACK: the two answers, the versions they were measured on, and any re-estimate. No PR is
needed if nothing changed; open one carrying a short findings note if anything did.
