...
test('the ladder is exactly the credit rungs — not everything named Papic', () => {
  ...
  */
  // ⚠ ONE CODE PER LINE, DELIBERATELY. gitleaks reads several upper-case
  // underscore tokens on ONE line as a `generic-api-key` and fails the build —
  // it did exactly that here. Splitting is the fix; a .gitleaksignore entry
  // would be a standing bill for a false positive nobody re-checks.
  const RUNGS = [
    'PAPIC_GUEST',
    'PAPIC_GUEST_100',
    'PAPIC_GUEST_50K',
