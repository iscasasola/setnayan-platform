 ...
 * `--m-orange` (Atelier kit gold) with a fallback, and is frozen to a static
...
export function LoadingNarration({
  ...
  return (
    <div className={`flex items-center justify-center gap-2.5 ${className}`.trim()}>
      <span
        aria-hidden
        className="h-4 w-4 shrink-0 animate-spin rounded-full border-2 border-ink/15 border-t-[var(--m-orange,#A9834B)]"
      />
      <LoadingStatus
