-- ============================================================================
-- CUSTOMER CARD RESPINE — PR-5 · BUDGET BAND (couple opt-in · range-only)
-- (corpus design source: 03_Strategy/Customer_Card_Prototype_2026-07-03.html)
-- Owner-approved 2026-07-03: opt-in, shown as a RANGE, never an exact number,
-- framed to couples as "get more accurate quotes, faster".
--
-- Two deliverables:
--   1. events.share_budget_band — host-controlled, per-event, default FALSE
--      (opt-IN). The host flips it on the couple's Budget surface; nothing is
--      shared until they do.
--   2. get_vendor_event_brief — the stage-aware brief (booked | inquiry) gains
--      ONE new key, "budget_band", in BOTH stage payloads. It is NULL unless
--      the host opted in AND the couple has a budget allocation for the calling
--      vendor's category(ies). When present it is a RANGE only:
--        { "lo_centavos": <band floor>, "hi_centavos": <band ceiling> }
--      The exact allocation is NEVER recoverable from the band (see formula).
--
-- Everything else about get_vendor_event_brief is preserved VERBATIM from
-- 20270507380212_customer_card_schema.sql — the vendor-org resolution, the
-- booked/inquiry stage gate, the disclosure-ladder inquiry payload, the full
-- booked payload, and the SECURITY DEFINER / STABLE / search_path hardening.
...
-- ----------------------------------------------------------------------------
-- 1 · events.share_budget_band — opt-in flag (default OFF)
-- ----------------------------------------------------------------------------
ALTER TABLE public.events
  ADD COLUMN IF NOT EXISTS share_budget_band BOOLEAN NOT NULL DEFAULT FALSE;

COMMENT ON COLUMN public.events.share_budget_band IS
  'Host opt-in (default FALSE): when TRUE, a vendor the couple is talking to sees a ROUNDED RANGE for their own category''s budget allocation on the Customer Card — never the exact figure. Written by the host on /dashboard/[eventId]/budget under their own session (events host RLS). Read only inside get_vendor_event_brief. Customer Card respine PR-5, owner-approved 2026-07-03.';

-- ----------------------------------------------------------------------------
-- 2 · get_vendor_event_brief — add "budget_band" to both stage payloads
-- ----------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.get_vendor_event_brief(UUID) IS
  'Stage-aware Vendor Event Brief (Customer Card respine). BOOKED vendors get the full aggregates-only brief; ACCEPTED-inquiry vendors get a LIMITED quote-relevant payload (city-grain location, pax totals, style, no timeline/seat/dietary). Both stages now carry "budget_band" — NULL unless the host opted in (events.share_budget_band) AND the couple allocated budget to the vendor''s category(ies); when present it is a ROUNDED RANGE {lo_centavos,hi_centavos} from which the exact figure is not recoverable. Top-level "stage" key = booked|inquiry. Guest PII never crosses.';
