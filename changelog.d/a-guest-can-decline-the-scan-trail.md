...
**Verified:** `tsc --noEmit` exit 0 · full unit suite green (11,831) · 25 new tests (17 lib + 8 component) · every guard mutation-tested — 13 sabotages applied one at a time, all 13 red, tree restored green after each. One of them was GREEN on the first run and is fixed here: the mount guard sliced FORWARD from `<ScanTrailNotice>`, but a gate is written BEFORE the element it gates, so wrapping the switch in a ternary left it passing. The window was on the wrong side.
