-- public_venue_scene v7 — carry the entrance KIND + depth to the public guest
-- walk so a couple's "Walk-through" entrance renders as a tunnel (not a door)
-- on their published venue page. The 2D editor + 3D lab already persist
-- event_floor_plan.entrance_kind / entrance_depth_m; this exposes them in the
-- public payload's floor.entrance block. Non-PII room geometry, matching the
-- existing entrance x/y — no token gate.
--
-- Idempotent CREATE OR REPLACE. IDENTICAL to the v6 (vendor-event-scope)
-- definition EXCEPT two added keys ('kind', 'depthM') on the entrance
-- jsonb_build_object. Signature unchanged (text, text); STABLE SECURITY DEFINER
-- + search_path + the v6 booth-vendor event-scope join all preserved.
