...
export default async function WelcomePage({ params, searchParams }: Props) {
  ...
  return (
    <DoorShell
      ...
      title={`You are the +1 of ${primaryName}`}
      sub={
      <>
        {primary?.first_name ?? 'They'} didn&rsquo;t have your details yet when they sent
        in their RSVP, so let&rsquo;s get you set up. This takes 10 seconds.
      </>
      }
      meta={`${event.display_name} · ${formatEventDate(event.event_date)}`}
    >
