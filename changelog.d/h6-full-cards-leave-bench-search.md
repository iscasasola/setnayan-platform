...
- **`public.service_cards_unbookable_on(uuid[], date[], boolean)`** (migration
  ...
  booking would. It is granted to `authenticated` only and capped at 100 cards
  and 31 dates. It returns no labels and no counts.
...
- `tests/db/a-full-card-leaves-bench-search.db.test.ts` runs a differential
  ...
  caps, anon denial, and "writes nothing". The RLS half of the tripwire lives
  here too.
