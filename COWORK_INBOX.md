...
## [PENDING] 2026-06-04 — Free monogram animations (5 effects) + paid SKU repositioning (0037)

**Why:** Owner locked monogram animation as a FREE feature. Free set = 5 effects auto-matched per lockup design (infinity→Trace · bar→Letter-by-letter · duo→Bloom · framed→Ink bleed · script→Living gold). Shipped in onboarding (CSS-only, ₱0).

**Spec corpus updates (owner via Cowork):**
1. **`0037_bespoke_monogram`** — add the FREE animation tier: 5 effects, auto-matched per design, pure CSS / no per-render cost. The paid Animated Monogram (₱2,499) is **repositioned** — it no longer sells "animation" generically; it sells the **bespoke unique mark** + the **7 premium effects** (Gold shimmer, Glow pulse, Ring spin-in, Unveil, Drop & settle, Handwritten, Fade & rise) + cross-surface propagation.
2. **Decision-log row:** "Monogram animation = FREE (5 auto-matched effects); paid SKU keeps bespoke + 7 premium effects + propagation."
...
1. **`0000_app_shell_and_navigation.md`** §2.5 — picker corrected to **all 9 event types live** (roster = `event-types.ts`, incl. Gender Reveal; Anniversary/Graduation/Reunion are enum-only); no coming-soon tier, no notify. ✅
2. **`DECISION_LOG.md`** — "keep everything live" decision row added (all 9 event types + all 6 religions stay live; not re-gated). ✅
3. **`0043_wedding_type_picker.md`** — `chinese` added to Axis A + a "Per-religion updates" section (Chinese active; the `/admin/wedding-types` vendor-readiness gate; the `/admin/wedding-traditions` DB-backed traditions guide). ✅
4. **`0023_admin_console.md`** — Wedding types (#895) + Wedding traditions (#898) Directory surfaces added; surface count 29 → 31. ✅
5. **.docx mirrors** regenerated for 0000 / 0023 / 0043 (pandoc). ✅

**Still the owner's to do (not a spec edit):** validate the per-religion traditions *content* (esp. INC / Muslim / Cultural / Chinese) at `/admin/wedding-traditions` — see the "VALIDATE CONTENT" item below.

The individual `[PENDING]` picker/per-religion items below (#882/#884/#885/#888/#889/#890/#895/#898) are **superseded by this — all done**; flip them to `[DONE]` or delete at your convenience.
...
2. **`0044_per_category_schemas`** — under `faith_compatibility` couple-side default filtering, add the new ceremony_type → filter mappings: `jewish` → `kosher_certified OR kosher_compatible`; `born_again` → alcohol-free. The broad `compatible_ceremony_types[]` vendor-fit gate already covers all 8; this documents the per-attribute deep filter for V1.1+ (same refinement status as the existing muslim/inc auto-filters).
3. **Spec 0000 (onboarding)** — the ceremony-tradition screen shows 8 chips in a 4×2 grid; all selectable.
4. **`DECISION_LOG.md`** — append: `| 2026-06-04 | Wedding tradition roster locked at 8 — Jewish + Born Again added as fully selectable; Born Again split from Christian; onboarding chips → 4×2 grid; wired into vendor compatible_ceremony_types + /vendors faith filter | 0043 + 0044 + onboarding + migration 20260808000000 |`

**Follow-up (in-app, not spec):** the new `/admin/wedding-traditions` guide editor (from the same-day "admin-editable wedding traditions" PR) has **no "what to expect" guide content for Jewish or Born Again** — the code `WEDDING_TRADITIONS_GUIDE` only covers the prior religions, so their `/paperwork` guide is empty until an admin authors it there (needs clergy validation, per that PR's honesty note).

**Owner action:** push migration `20260808000000_add_jewish_bornagain_ceremony_types.sql` (`supabase db push`) — *done in-session by Claude Code immediately after merge; confirm before marking done.*

**Cross-ref:** `CHANGELOG.md` 2026-06-04 "feat(0043,0044): lock 8 wedding traditions". Related to the Chinese-activation item below and the admin-editable-traditions item directly under this one.
