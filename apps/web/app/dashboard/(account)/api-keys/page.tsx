...
export default async function ApiKeysPage({ searchParams }: Props) {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      <Link href="/dashboard/profile" className="sn-chip sn-press mb-4 w-fit">
      ...
      {justCreated ? (
        <section className="mb-6 space-y-3 rounded-tile border border-success-300/60 bg-success-50/80 p-5">
          <div className="space-y-1">
            <p className="sn-eye text-success-900">
              Save this key now
            </p>
          ...
          <div className="sn-row flex flex-wrap items-center gap-2 p-3">
      ...
      <section className="sn-tile mb-8 space-y-3">
        <h2 className="sn-sec">Create a key</h2>
      ...
      <section className="space-y-3">
        <h2 className="sn-sec">Your keys ({keys.length})</h2>
        {keys.length === 0 ? (
          ...
          <ul className="space-y-2">
            {keys.map((k) => (
              <li
                key={k.api_key_id}
                className={`sn-row flex flex-wrap items-start justify-between gap-3 p-4 ${
                  k.revoked_at ? 'opacity-70' : ''
                }`}
              >
      ...
      <section className="sn-tile mt-10 space-y-3 border-dashed">
        <p className="sn-eye">Try it</p>
...
function ScopeCheckbox({
  ...
  return (
    <label
      className={`sn-row flex items-start gap-3 p-3 ${
