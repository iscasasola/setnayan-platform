# E2 — overlays on the canvas, and the drift guard

**Model:** Sonnet 5 · **Effort:** high · **Wave:** after E1 · THE RISKY ONE
...
TASK — E2: re-draw the overlays on the canvas without creating a second source of truth.

🚩 SCOPE § 5 CALLS THIS "THE ONE GENUINELY HARD PART" AND "THE SURFACE MOST LIKELY TO BE LEFT
UNCOVERED". It is the session that protects the ₱2,500: overlays are the paid layer, and a
canvas that quietly stops drawing POWERED BY SETNAYAN gives every free host an unbranded
broadcast.

THE OVERLAYS: monogram · lower third · event QR · POWERED BY SETNAYAN. They are React DOM
today, so the canvas encoder must RE-DRAW them — a second renderer for the same fact.

🔑 THE MITIGATION THAT MAKES THIS TRACTABLE — the DECISION stays single-sourced and
server-side; only the DRAWING is duplicated.
  · The encoder must CONSUME `ProgramFrame.overlay` / `WatermarkReason` and the air decision
    (`decideWatermark`, `decideProgramAir` / `programSourceAllowed`) exactly as the pop-out
    does, and NEVER re-derive them. The pop-out's own docblock says why: it "has no
    entitlement context and would be the easiest surface to leave uncovered".

GUARD — this is the deliverable as much as the renderer:
  · Assert the canvas renderer handles EVERY `WatermarkReason` variant, so adding a variant
    FAILS THE BUILD rather than silently airing unbranded. An exhaustive switch with a
    `never` check is the shape.
  · Assert the renderer contains no call to `decideWatermark` or any re-derivation — it reads
    the flag it was given.
  · MUTATION-TEST both: removing a variant, and re-deriving the decision, must each turn red.

Full suites. COMMIT BEFORE A LONG VERIFY.
