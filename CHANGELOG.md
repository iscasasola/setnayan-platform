...
## 2026-06-13 · feat(seat-pass): personalized seat pass + public QR resolver + arrival bloom (seat-finding PR 4 of 6)

Adds the paid Seat Pass — the surface the **Custom-QR Guest** SKU's branded per-guest / per-table QR codes point at. Strictly **additive**: `find-my-table` + its `INDOOR_BLUEPRINT` gate are **untouched**; no live SKU is retired. Reuses PR 3's `checkOrderOwnership` + the activation dispatcher (no duplication). **No migration** — the arrival bloom reads the already-deployed `guest_checkins.checked_in_at`; tokens reuse the existing `guests.qr_token` / `event_tables.qr_token`. Migration slot `20261218000000` is intentionally left unconsumed (reserved for owner-gated bloom analytics only).

- **New `/[slug]/seat?t={token}`** (`app/[slug]/seat/page.tsx`) — one route, two token kinds, dual lookup. `t` is **either** a `guests.qr_token` (→ **personal seat pass**: name · table · `seat_number` marker · wayfinding route · arrival bloom) **or** an `event_tables.qr_token` (→ **public table view**: label · occupants by first name + last initial · route to that table — mirrors the physical table sign, no per-guest PII). Neither → `notFound()`. Both branches **gate on `CUSTOM_QR_GUEST` BEFORE any token lookup** (`eventOwnsCustomQrGuest`); unowned → friendly "ask the couple" card, nothing leaked. `force-dynamic`. Reuses `WayfindingMap` + `fetchEntrance`/`EntrancePos` from `lib/indoor-blueprint.ts` unchanged; self-contained `SeatPassShell` (does not import the `[slug]` page).
- **New `/[slug]/seat/claim` route handler** (`.../seat/claim/route.ts`) — the personal-QR cookie-set hop (Next.js permits cookie writes only in Route Handlers / Server Actions). Validates the **guest** token via the admin client, `setGuestSession(...)`, records a `scan_events` `{ entry: 'personal_qr_scan' }`, then redirects to the clean `…/seat?t={token}` pass URL. Printed personal QRs encode the `/claim` URL; table QRs encode the direct stateless `/seat` URL. The pass page itself never depends on the cookie.
- **New `arrival-bloom.tsx` client island** (`.../seat/_components/`) — one-shot bloom on load / on the explicit "Show my seat" tap. Richness ladder, graceful-degrade: event owns `ANIMATED_MONOGRAM` → `AnimatedMonogramHero motion="bloom"` (forced to the bloom signature for the *arrival* moment, ignoring `events.monogram_motion_key`); else static `EventMonogram`. `arrived` (from `guest_checkins`) only swaps the copy. `prefers-reduced-motion`-safe. `// PR5:` marker for live Realtime repaint (out of scope). `hasPakanta` branch + placeholder wired but **always false** this PR.
- **New `lib/seat-pass.ts`** — `CUSTOM_QR_GUEST_SERVICE_KEY` (promotes the previously-inline `SKU_CODE` literal to a shared const), `eventOwnsCustomQrGuest` (delegates to `checkOrderOwnership`), and `eventOwnsPakanta` — an **inert stub returning `false`** (Pakanta is `not_built`: no table, no migration, no order path; the bloom's song branch is wired but harmless until the SKU ships).
- **`lib/sku-activation.ts`** — registered `EXACT_HOOKS['CUSTOM_QR_GUEST']` in place of PR 3's `PAPIC_SEATS` placeholder comment. **Rename flagged for the record:** the slot was sketched as `PAPIC_SEATS` but the seat pass resolves + gates on `CUSTOM_QR_GUEST` (PAPIC_SEATS is its own already-`live` photo-crew SKU), so the hook binds to `CUSTOM_QR_GUEST`. Idempotent + defensive: stamps `event_tables.qr_published_at` on **null-only** rows (tokens already default-exist from row creation — a publish stamp, not a reset) + appends a `service_activated` ledger row. NEVER throws (dispatcher contract); `approvePayment` untouched.
- **`app/[slug]/page.tsx`** — additive, separately-gated **"Your seat pass"** link next to the existing find-my-table link, shown only when the event owns `CUSTOM_QR_GUEST` and the guest has a `qr_token`; deep-links to `/seat/claim?t={qr_token}`. The find-my-table link is byte-for-byte unchanged.
- New unit suite `lib/seat-pass.test.ts` (11 cases). Ownership half tests `eventOwnsCustomQrGuest` (paid/submitted/none/refunded/`42P01`) + `eventOwnsPakanta` always-false (with a throwing client proving the stub never queries). Activation half tests the hook **contract** via a dependency-free replica (idempotent null-only stamp · single `service_activated` ledger · no-eventId no-op · never-throws) — **deviation:** `lib/sku-activation.ts` is *not importable* under the plain node test runner (it transitively imports a `'use server'` module → `next/headers`), like every other dependency-light tested lib; the replica is kept line-for-line in sync with the real hook.

**Owner sign-off needed at review (surfaced, not built):**
1. **IB ↔ Custom-QR reconciliation.** `/find-my-table` (gated `INDOOR_BLUEPRINT` ₱1,499) and `/seat` (gated `CUSTOM_QR_GUEST` ₱1,499) now both reuse the same `WayfindingMap` — two paid paths to overlapping wayfinding. The corpus direction is "retire IB → Custom-QR," but retiring a live SKU needs sign-off. PR 4 ships **Option A** (both side-by-side, nothing touched). **Recommendation: approve Option B next** (one-line OR-gate so `find-my-table` accepts `IB_owned || CUSTOM_QR_owned` — IB buyers get the richer pass free, zero breaking change/migration, both SKUs still sellable); reserve **Option C** (true IB retirement + order backfill) for a deliberate comms-backed cutover.
2. **Activation-hook label rename** `PAPIC_SEATS` placeholder → `CUSTOM_QR_GUEST` (on the record above).

- Verified: `pnpm typecheck` ✅ · `pnpm lint` ✅ · `pnpm build` ✅ · `pnpm test:unit` ✅.

SPEC IMPACT: corpus 0008 (seating · entrance→table wayfinding now also reachable via the Custom-QR seat pass) + 0024/Custom-QR (the per-guest/table QR resolves to a personalized pass + arrival bloom). The IB↔Custom-QR reconciliation (above) is flagged for owner sign-off. No SKU/pricing/schema change; no migration. To be logged at the bottom of corpus `DECISION_LOG.md` (2026-06-13).
