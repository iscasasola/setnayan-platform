...
/*
  ⚠ FOUR LEVELS, NOT THREE (2026-08-15). This page moved into the `app/(shell)/`
  route group, so it sits one directory deeper on disk than its URL suggests —
  `/privacy/google-access` is served from
  `app/(shell)/privacy/google-access/page.tsx`. A route group is invisible in
  the URL and present in the path; a relative root that was correct before the
  move silently resolved to `apps/web/app/lib/…` and every read ENOENTed.
*/
...
for (const which of ['openGraph', 'twitter'] as const) {
  test(`the page's ${which} override restates every key the root layout sets`, () => {
    ...
    assert.ok(
      ...
      `app/(shell)/privacy/google-access/page.tsx declares no \`${which}\` object. Next does ` +
    ...
    for (const key of layoutKeys(which)) {
      assert.ok(
        ...
        `app/(shell)/privacy/google-access/page.tsx overrides \`${which}\` without \`${key}\`, ` +
