# E6 — the stream key never reaches the renderer

**Model:** Sonnet 5 · **Effort:** high · **Wave:** parallel with E1–E4
...
TASK — E6: a bearer credential for a channel OTHER COUPLES' weddings stream on.

Scope § 6 is the specification. The key must:
  ...
  · be redacted in any RTMP URL printed in diagnostics — `rtmp://…/live2/****`;
  · be revocable, because a leak means resetting it on a pool channel that other weddings use.

🔑 THE PRECEDENT TO FOLLOW, NOT A SHORTCUT: `live_studio_channel_grants` is RLS-ENABLED WITH NO
POLICY — service-role only — specifically because widening `oauth_grants` would have exposed a
platform credential row-wise to event members. Follow that reasoning.

⚠ A WEBVIEW IS NOT A TRUST BOUNDARY. The renderer runs the same JS a browser would; anything
handed to it is readable. "It is only in memory" is not containment.

GUARD: no path from the key to the renderer; a redaction test over the diagnostic strings; the
key's fetch is server-side. MUTATION-TEST — passing the key into the webview, and printing an
unredacted URL, must each turn red. Full suites. COMMIT BEFORE A LONG VERIFY.
