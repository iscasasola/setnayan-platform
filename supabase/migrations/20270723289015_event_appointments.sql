-- ============================================================================
-- RECOVERY RE-ADD (2026-07-11). This migration's SQL was originally committed
-- as 20270713200000_event_appointments.sql (commit a8a923f03), applied to prod
-- via the auto-apply workflow, then the FILE was later removed from main
-- (its hand-typed round prefix violated the allocator rule) — leaving prod
-- AHEAD of main with no migration of record. This re-adds the exact original
-- SQL under a proper allocator prefix. It is IDEMPOTENT (CREATE TABLE IF NOT
-- EXISTS · DROP POLICY IF EXISTS · CREATE INDEX IF NOT EXISTS), so db push
-- re-applying it against prod (where the table already exists) is a safe no-op;
-- a fresh DB (CI / local / new env) gets the table for the first time.
-- Validated as a rolled-back re-apply against live prod before commit.
-- ============================================================================
