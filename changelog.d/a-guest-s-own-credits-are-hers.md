...
SPEC IMPACT: **Applied to the corpus** (direct-edit authorization, 2026-06-04). No SKU, price or schema-shape change — the code half only makes an existing owner ruling (2026-08-28 § b) true in the gate. But the corpus's own S5 note described a mechanism that is **not** the shipped one, and was corrected in place:

- `~/Documents/Claude/Projects/Setnayan/WHATS_NEXT_Shots_Per_Guest_SESSIONS_2026-08-28.md` § S5 — the note read *"✅ `papic_dedicate_shots` takes a TARGET, not a delta … this is that call in the pot direction. **Nothing new.**"* **Wrong in both halves.** (1) At purchase `papic_dedicate_shots` is not involved at all: the two choices are two ORDER KINDS resolved at approval — `grantPapicPassPoints` writes a shared grant for a `pool_topup`, `papic_grant_camera_points` a seat grant for a `one_reload`. (2) The *give the unspent part back later* half is **not buildable that way**: `papic_dedicate_shots` reads and writes `papic_seat_allocations` only — the host's hand-out layer — and cannot reach a guest's *grant*; on a camera whose dedicated balance is all grants the allocation row is `0` and the function rejects `p_points < 0` outright. ⇒ **A buyer-side release of unspent bought credits remains UNBUILT and needs its own primitive.** ⏭ Owner call whether it is wanted at all. S5 is also marked gate-half-built and carries the open host-allocation question.
- `~/Documents/Claude/Projects/Setnayan/DECISION_LOG.md` — one dated row with the defect, the derivation, both open owner questions and the mutation finding.

⚠ Left **uncommitted** in the corpus repo on purpose: another session has an in-flight uncommitted line in `DECISION_LOG.md`, and committing would sweep it in. No `.docx` mirror exists for the `WHATS_NEXT_*` docs, so no pandoc regeneration was needed.
