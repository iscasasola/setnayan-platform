...
test('eventOwnsCustomQrGuest: a still-in-reconciliation submitted row → false (feature gate needs approval)', async () => {
  // CUSTOM_QR_GUEST gates a PAID feature surface, so eventOwnsCustomQrGuest
  // delegates to the admin-approved eventSkuActive() reader (checkOrderActive ⇒
  // paid/fulfilled only). A pending 'submitted' order stays dark until the
  // Setnayan team verifies payment — same handshake as every paid-feature gate.
...
test('eventOwnsCustomQrGuest: a fulfilled row confers active entitlement (true)', async () => {
  const supabase = makeOwnershipSupabase({ data: [{ status: 'fulfilled' }], error: null });
  assert.equal(await eventOwnsCustomQrGuest(supabase, 'evt_1'), true);
});
