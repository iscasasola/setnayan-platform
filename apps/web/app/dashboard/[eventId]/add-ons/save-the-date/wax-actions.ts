 ...
 * /dashboard/[eventId]/add-ons/save-the-date/stamp).
 *
...
function backToMaker(eventId: string, params?: Record<string, string>): never {
  ...
  redirect(`/dashboard/${eventId}/add-ons/save-the-date/stamp${qs}#wax-maker`);
}
...
export async function saveWaxSeal(formData: FormData): Promise<void> {
  ...
  revalidatePath(`/dashboard/${eventId}/add-ons/save-the-date/stamp`);
