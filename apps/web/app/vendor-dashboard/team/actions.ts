...
export async function inviteVendorTeamMember(formData: FormData) {
  ...
  // cap = the tier's base agentAccounts + any paid extra seats (Enterprise
...
// ── Extra team seats (Enterprise ₱250/28d add-on · owner 2026-07-02) ─────────
...
/**
 * Buy one extra team seat beyond the tier's base cap. Enterprise-only (the base
 * ladder tops out at 10; extra seats are the paid escape valve). Apply-then-pay:
 * an `orders` row keyed `vendor_extra_seat__{vendor_profile_id}` + a pending
 * `payments` row so it lands in /admin/payments. On admin approval the
 * sku-activation hook recomputes vendor_profiles.extra_agent_seats and the seat
 * becomes usable. The ongoing ₱250 folds into the Enterprise renewal in PR-B.
 */
export async function buyExtraSeat(formData: FormData) {
  const { supabase, ctx, currentUserId } = await ensureAdmin();

  if (ctx.tierState !== 'enterprise') {
