 ...
 *
 * ⚠ THE MARK ITSELF MOVED ON (MB20). What this file pins is that the vendor
 * upload path produces a marked, decodable image at all; the mark's own
 * geometry — that it fits its plate, that the seal goes on celebrations and the
 * stamp on everything else — is pinned next door in lib/mark-fits-and-marks
 * .test.ts, which is where the overflow bug is asserted against.
 *
...
test('the wordmark is the URL, spelled out', () => {
  // ⚠ REGENERATED BY MB20, AND THE OLD LINE IS WHY THIS COMMENT EXISTS. This
  // asserted `WATERMARK_TEXT === 'SETNAYAN'` from 2026-09-04 until the owner
  // directive that every inspiration photo must carry an address somebody can
  // actually type. It went red on the rename, which is the test doing its job —
  // the brand lock it holds is "spelled out, never STNYN", and that survives
  // the change intact because the URL contains the full word.
