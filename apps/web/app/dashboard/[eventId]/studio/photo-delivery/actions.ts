...
> {
  ...
  if (!membership || membership.member_type !== 'couple') {
    return {
      ok: false,
      redirectTo: `/dashboard/${eventId}/studio/photo-delivery?sync_mode_error=not_a_couple`,
    };
  }
...
async function updateSyncMode(
  ...
  if (error) {
    return {
      ok: false,
      redirectTo: `/dashboard/${eventId}/studio/photo-delivery?sync_mode_error=${encodeURIComponent(
...
export async function setPhotoDeliverySyncModeManual(formData: FormData) {
  ...
  revalidatePath(`/dashboard/${auth.eventId}/studio/photo-delivery`);
  redirect(
    `/dashboard/${auth.eventId}/studio/photo-delivery?sync_mode_set=manual_release`,
  );
}
...
export async function setPhotoDeliverySyncModeAuto(formData: FormData) {
  ...
  revalidatePath(`/dashboard/${auth.eventId}/studio/photo-delivery`);
  redirect(
    `/dashboard/${auth.eventId}/studio/photo-delivery?sync_mode_set=auto_sync`,
  );
}
...
export async function releasePhotoDelivery(formData: FormData) {
  ...
  if (!result.ok) {
    redirect(
      `/dashboard/${auth.eventId}/studio/photo-delivery?release_error=${encodeURIComponent(
  ...
  revalidatePath(`/dashboard/${auth.eventId}/studio/photo-delivery`);
  redirect(
    `/dashboard/${auth.eventId}/studio/photo-delivery?release_started=1${
...
export async function disconnectPhotoDelivery(formData: FormData) {
  ...
  revalidatePath(`/dashboard/${auth.eventId}/studio/photo-delivery`);
  redirect(`/dashboard/${auth.eventId}/studio/photo-delivery?disconnected=1`);
}
