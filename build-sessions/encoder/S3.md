# S3 — the audio mixer that does not exist, and the master clock

**Model:** Opus 5 · **Effort:** high · **Wave:** after S1 · the B-remux branch point

Measured against `origin/main` @ `bd1c1a39f` on 2026-09-05. Re-fetch before you act.

Part of the S-series encoder plan — `build-sessions/encoder/README.md`. Supersedes E3's audio half (unbudgeted there).
...
TASK — S3: build programme audio from per-phone tracks, and make it the clock everything follows.

RULE 0 HITS (there is nothing to extend — verify that first and paste the grep):
  · `git grep -n "AudioContext" origin/main -- apps/web/lib apps/web/app/panood` → hits only in
    `mesh-call-webrtc.ts` and `reel-render.ts`. No mixer, no programme audio.
  · The pop-out just unmutes the on-air `<video>` (and BOTH in split — a defect, not a spec).
  · Phone audio: `lib/panood-webrtc.ts` publishes A/V per phone; mic may be refused → video-only.
  · The controller comments that the mixing point "does not exist (phase 2)". This is phase 2.

BUILD `apps/web/lib/encoder/audio-mixer.ts` + `audio-tap.worklet.ts`:
  · ONE `AudioContext({ sampleRate: 48000 })`, created on the go-live click (user gesture).
  · One `MediaStreamAudioSourceNode` per camera stream that HAS an audio track, each → its own
    `GainNode`. On a cut: on-air gain → 1, others → 0, `linearRampToValueAtTime` over 5 ms.
  · A `ConstantSourceNode` at 0 always summed, so the graph produces frames when the on-air
    phone has no mic — YouTube requires an audio track; silence is not a dead stream.
  · Split mode: PRIMARY only.
  · Laptop mic: NOT an input. No permission prompt, no echo path.
  · Sum → `AudioWorkletNode("setnayan-tap")`: posts 128-frame Float32 quanta with `currentFrame`
    to S1's worker. Worker packs 1024 frames → `AudioData{ format:'f32-planar', sampleRate:48000,
    numberOfFrames:1024, timestamp }` → `AudioEncoder{ codec:'mp4a.40.2', sampleRate:48000,
    numberOfChannels:2, bitrate:128000 }`. Capture `decoderConfig.description` (AudioSpecificConfig)
    from the first chunk — S4/S5 ship it to Rust.
  · Keep a `MediaStreamAudioDestinationNode` sibling output. It is unused on Path A; it is what
    B-remux would feed to `captureStream()`. One line, documented.
  · MASTER CLOCK: `AudioContext.currentTime` / worklet `currentFrame`. Replace S1's timer seam:
    the worker derives the video tick from audio frames — every 12.8 quanta (accumulate the
    fraction; never `setInterval`, never rAF — both stop when minimised).
    `VideoFrame.timestamp = frameIndex × 1e6 / 48000` µs. Continuous across cuts because the
    SOURCE never changes — only gains do.

GUARDS (mutation-test, print counts):
  · consecutive `AudioData.timestamp` deltas === 1024/48000 s across a cut (no discontinuity).
  · a camera with no audio track still yields AudioData (silence source). Sabotage: remove the
    ConstantSourceNode → red.
  · S1's timer path is gone: grep the worker for `setInterval|requestAnimationFrame` → 0 hits.

EVIDENCE: a 30-minute run in the real app, cuts every 10 s between two phones (one mic-less),
logging |videoPTS − audioPTS| — must stay < 40 ms throughout. Paste the max.
