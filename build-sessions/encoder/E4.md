# E4 — frames reach Rust, and a slow wire degrades instead of stalling

**Model:** Sonnet 5 · **Effort:** high · **Wave:** after E3
...
TASK — E4: the IPC transport from webview to Rust, with backpressure.

🔑 THE PROPERTY THAT MATTERS MORE THAN THROUGHPUT: when the wire cannot keep up, the pipeline
must DROP FRAMES AND CARRY ON. It must never block the webview — a stalled composite freezes
the CONTROLLER, which is the operator's only view of their own wedding.

  · Bounded queue with a drop policy, not an unbounded buffer. An unbounded queue does not
    avoid the failure, it just converts a visible stutter into an invisible memory climb and
    a crash two hours later, at the reception.
  · Drops must be COUNTED and surfaced, not silent — they are the earliest signal the venue
    uplink is failing, and the operator can act on it (lower the bitrate) if they are told.
  · Encoded chunks only. Do not ship raw frames across IPC.

⚠ THIS REPO'S SIGNATURE DEFECT IS A FAILURE THAT RENDERS AS SUCCESS. A dropped-frame counter
that only reaches a log is the same defect in a new place. It belongs beside the ingest-health
strip LS4 shipped (`lib/live-studio-ingest-health.ts`) — extend that, do not build a second
health surface that can disagree with it.

GUARD: a saturated queue drops and continues; the drop count is exposed to the render path;
nothing awaits the consumer on the producer's path. MUTATION-TEST — making the queue unbounded,
and swallowing the drop count, must each turn red. Full suites. COMMIT BEFORE A LONG VERIFY.
