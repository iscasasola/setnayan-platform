...
/** orders.service_key / package_code for the "Unlock all of Papic" bundle (₱15,000). */
export const PAPIC_UNLOCK_ORDER_KEY = 'PAPIC_UNLOCK';

/**
 * THE camera-allowance grant (owner-locked 2026-06-26 · completes the deferred
 * piece of the "Unlock all of Papic" bundle): owning PAPIC_UNLOCK grants free,
 * UNCAPPED Unli cameras. So every Unli-tier seat shoots WITHOUT its own paid
 * per-camera order — the capture gate ORs this onto papicCameraOrderPaid. Read
 * on an ADMIN client (the claimer isn't an event member). eventSkuActive is
 * bundle-aware and requires paid/fulfilled, so an apply-then-pay PAPIC_UNLOCK
 * stays dark until the Setnayan team confirms — consistent with the per-camera
 * paid gate. Fail-CLOSED: any error returns false.
 *
 * Scope: UNLI tier only. Roll/Ltd cameras are NOT unlocked by the bundle (Unli
 * is the premium tier the bundle covers, and it's strictly better — so there's
 * no reason to shoot Roll once Unlock All is owned).
 */
