...
--
-- KEEP THIS MIGRATION IDEMPOTENT (it may be re-applied — the table already
-- exists in prod, so on prod this whole file is a no-op):
...
-- when the CREATE TABLE above just ran, but it keeps the schema correct if an
-- older partial table already exists.
ALTER TABLE public.panood_broadcasts ADD COLUMN IF NOT EXISTS broadcast_id       text;
ALTER TABLE public.panood_broadcasts ADD COLUMN IF NOT EXISTS stream_id          text;
ALTER TABLE public.panood_broadcasts ADD COLUMN IF NOT EXISTS stream_key         text;
ALTER TABLE public.panood_broadcasts ADD COLUMN IF NOT EXISTS ingestion_url      text;
ALTER TABLE public.panood_broadcasts ADD COLUMN IF NOT EXISTS status             text;
ALTER TABLE public.panood_broadcasts ADD COLUMN IF NOT EXISTS scheduled_start_at timestamptz;
ALTER TABLE public.panood_broadcasts ADD COLUMN IF NOT EXISTS went_live_at       timestamptz;
ALTER TABLE public.panood_broadcasts ADD COLUMN IF NOT EXISTS ended_at           timestamptz;
ALTER TABLE public.panood_broadcasts ADD COLUMN IF NOT EXISTS created_at         timestamptz NOT NULL DEFAULT now();
ALTER TABLE public.panood_broadcasts ADD COLUMN IF NOT EXISTS updated_at         timestamptz NOT NULL DEFAULT now();

-- RLS enabled at CREATE TABLE time (canonical rule), in the SAME migration. No
-- policy is defined on purpose: the table carries the secret stream_key, so
-- anon/authed clients must never read it. lib/panood-broadcast.ts reads + writes
-- it via the admin (service-role) client only; the couple sees their stream key
-- through the server-rendered setup page, never a direct client query.
