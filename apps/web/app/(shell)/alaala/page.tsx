 ...
 *
 * ─── WHY THIS ONE IS NOT ON THE SHARED DOORWAY KIT (design#6) ────────────
 * Seven of the eight doorways mount `DoorwayPage`. This one deliberately does
 * not, and the reason is content, not effort: Alaala is the UMBRELLA over the
 * other five, so it has no how-it-works panel, its differentiator carries no
...
export default function AlaalaLandingPage() {
  return (
    ...
    */
    <>
      {/*
        ⚠ STILL NOT ON THE DOORWAY KIT, and that is a SEPARATE question from
        wearing the shared chrome. Porting onto `DoorwayPage` would delete the
        "Read the whole story" CTA (its `closing` takes one href) and strip the
        links off all five pillar cards (`DoorwayStep` has no href). That is
        redrawing. Guarded by `doorway-shell.test.ts`.
      */}
